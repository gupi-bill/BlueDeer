...
class Widget(ABC):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"name": self.name, "title": self.title, "width": self.width, "height": self.height}
...
class WidgetRegistry:
    ...
    def render_all(self) -> str:
        ...
        for w in self._widgets.values():
            rendered = w.render()
            parts.append(f'<div class="widget widget-{w.width}" data-widget="{w.name}">')
...
def get_stats(metric: str, period: str = "24h") -> dict[str, Any]:
    now = time.time()
    period_sec = {"1h": 3600, "6h": 21600, "24h": 86400, "7d": 604800}.get(period, 86400)
    cutoff = now - period_sec
    base: dict[str, Any] = {"metric": metric, "period": period, "period_sec": period_sec, "cutoff": cutoff, "value": 0}
    if _harness is None:
        return base
    try:
...
class StatsWidget(Widget):
    def __init__(self, name: str, metric: str, title: str = "", period: str = "24h") -> None:
        super().__init__(name, title)
        self.metric = metric
        self.period = period
...
class ChartWidget(Widget):
    def __init__(self, name: str, metrics: list[str], title: str = "", period: str = "24h") -> None:
        super().__init__(name, title)
...
async def dashboard(request: Request) -> str:
    ...
    if _harness:
        ...
        stats = {"total": s["total"], "success": s["success"], "failed": s["failed"], "pending": s["pending"]}
    ...
    try:
        ...
        health = [{"service": h.service, "status": h.status, "latency_ms": round(h.latency_ms, 1)} for h in mon.check_services()]
    except Exception:
        pass

    return HTMLResponse(content=_render("admin/dashboard.html",
        active="dashboard", stats=stats, schedule_count=schedule_count,
        webhook_count=webhook_count, resources=resources, health=health,
        dag_nodes=dag_nodes, dag_layers=dag_layers, dag_cycle=dag_cycle,
    ))
...
async def widgets_page(request: Request) -> str:
    ...
    return HTMLResponse(content=_render("admin/widgets.html", active="widgets", widgets=widgets, rendered=rendered))


@router.post("/widgets/add")
async def widgets_add(name: str = Form(...), widget_type: str = Form("text"), title: str = Form(""), metric: str = Form(""), text: str = Form("")) -> str:
...
async def tasks_page(request: Request) -> str:
    ...
    return HTMLResponse(content=_render("admin/tasks.html",
...
async def schedules_page(request: Request) -> str:
    jobs: dict[str, Any] = {}
    if _scheduler:
        jobs = {jid: {"cron": j.cron, "task_type": j.task_type, "enabled": j.enabled, "description": j.description}
                for jid, j in _scheduler.list_jobs().items()}
    return HTMLResponse(content=_render("admin/schedules.html",
        active="schedules", jobs=jobs,
    ))
...
async def schedules_create(
    request: Request,
    job_id: str = Form(...), cron: str = Form(...),
    task_type: str = Form("general"), assignee: str = Form(""),
    description: str = Form(""),
) -> str:
    if _scheduler is None:
        return '<div class="toast toast-error">Scheduler 未初始化</div>'
    job = JobDef(id=job_id, cron=cron, task_type=task_type, assignee=assignee, description=description)
...
def _schedule_row(job_id: str, j: JobDef) -> str:
    enabled = j.enabled
    badge = '<span class="badge badge-green">启用</span>' if enabled else '<span class="badge badge-gray">禁用</span>'
    toggle_btn = (
        ...
        if enabled else
        f'<button class="btn btn-sm" hx-post="/admin/schedules/{job_id}/enable" hx-target="closest tr" hx-swap="outerHTML">启用</button>'
    )
    return f"""<tr>
...
async def webhooks_page(request: Request) -> str:
    hooks: dict[str, Any] = {}
    if _webhook:
        hooks = {hid: {"url": h.url, "events": h.events, "enabled": h.enabled, "description": h.description}
                 for hid, h in _webhook.list_hooks().items()}
    return HTMLResponse(content=_render("admin/webhooks.html",
        active="webhooks", webhooks=hooks,
    ))
...
async def webhooks_create(
    request: Request,
    hook_id: str = Form(...), url: str = Form(...),
    events: str = Form(",".join(_ALL_EVENTS)),
    secret: str = Form(""), description: str = Form(""),
) -> str:
    if _webhook is None:
        return '<div class="toast toast-error">WebhookDispatcher 未初始化</div>'
    event_list = [e.strip() for e in events.split(",") if e.strip()]
    hook = WebhookDef(id=hook_id, url=url, events=event_list, secret=secret, description=description)
...
def _webhook_row(hook_id: str, h: WebhookDef) -> str:
    ...
    toggle_btn = (
        ...
        if enabled else
        f'<button class="btn btn-sm" hx-post="/admin/webhooks/{hook_id}/enable" hx-target="closest tr" hx-swap="outerHTML">启用</button>'
    )
    events_html = " ".join(f'<span class="badge badge-blue">{ev}</span>' for ev in h.events)
    return f"""<tr>
...
async def templates_page(request: Request) -> str:
    ...
    if _templates_engine:
        tmpls = [{"id": t.id, "type": t.type, "assignee": t.assignee, "tags": t.tags, "description": t.description}
                 for t in _templates_engine.list_templates()]
    return HTMLResponse(content=_render("admin/templates_page.html",
        active="templates", templates=tmpls,
    ))
...
async def dag_page(request: Request) -> str:
    ...
    return HTMLResponse(content=_render("admin/dag.html",
        active="dag", nodes=nodes, plan=plan, has_cycle=cycle is not None,
...
async def dag_add_node(request: Request) -> str:
    ...
    dag.add_node(
        form.get("id", ""),
        depends_on=[x.strip() for x in form.get("depends_on", "").split(",") if x.strip()],
...
async def monitor_page(request: Request) -> str:
    resources: dict[str, Any] = {"cpu_percent": 0, "memory_percent": 0, "disk": {"total_gb": 0, "used_gb": 0, "free_gb": 0, "percent": 0}, "timestamp": time.time()}
    health: list[dict[str, Any]] = []
    ...
    try:
        ...
        pass
    return HTMLResponse(content=_render("admin/monitor.html",
        active="monitor", resources=resources, health=health, alerts=alerts,
    ))
...
async def floorplan_page(request: Request) -> str:
    aggr: dict[str, Any] = {"success": 0, "failed": 0, "pending": 0, "total": 0, "health": "良好"}
    try:
        if _harness:
            aggr = _harness.aggregate()
    except Exception:
        pass
    return HTMLResponse(content=_render("admin/floorplan.html",
        active="floorplan", aggr=aggr,
    ))
...
async def agents_page(request: Request) -> str:
    ...
    try:
        ...
        pass
    return HTMLResponse(content=_render("admin/agents.html",
        active="agents", categories=categories,
    ))
...
async def agent_health_page(request: Request) -> str:
    return HTMLResponse(content=_render("admin/agent_health.html",
        active="agent-health",
    ))
...
async def alerts_page(request: Request) -> str:
    return HTMLResponse(content=_render("admin/alerts.html",
        active="alerts",
    ))
...
async def logs_page(
    ...
    viewer = LogViewer()
    viewer.reload()
    result = viewer.query(level=level or None, component=component or None,
                          keyword=keyword or None, action=action or None,
                          offset=offset, limit=limit)
    stats = viewer.stats()
    entries_data = result.get("entries", [])
    return HTMLResponse(content=_render("admin/logs.html",
        active="logs", log_stats=stats, entries=entries_data,
        total_entries=result.get("filtered", 0), offset=offset, limit=limit,
    ))
...
async def report_generate(
    ...
    gen = ReportGenerator()
    path = gen.generate(task_board=task_board, aggregate_stats=stats, trace_lines=trace_lines, fmt=fmt, title=title)
...
@router.get("/agent-health", response_class=HTMLResponse)
async def agent_health_page(request: Request) -> str:
    return HTMLResponse(content=_render("admin/agent_health.html", active="agent-health"))
...
async def dag_templates_page(request: Request) -> str:
    return HTMLResponse(content=_render("admin/dag_templates.html", active="dag-templates"))
...
async def room_breakroom(request: Request) -> str:
    return HTMLResponse(content=_render("admin/room_breakroom.html", active="floorplan"))
