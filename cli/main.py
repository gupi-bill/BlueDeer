...
class Color:
    ...
    @staticmethod
    def ok(msg: str) -> str: return f"{Color.G}{msg}{Color.N}"
    @staticmethod
    def warn(msg: str) -> str: return f"{Color.Y}{msg}{Color.N}"
    @staticmethod
    def err(msg: str) -> str: return f"{Color.R}{msg}{Color.N}"
    @staticmethod
    def info(msg: str) -> str: return f"{Color.B}{msg}{Color.N}"
    @staticmethod
    def dim(msg: str) -> str: return f"{Color.D}{msg}{Color.N}"
    @staticmethod
    def tag(t: str, msg: str) -> str: return f"[{Color.M}{t}{Color.N}] {msg}"
    @staticmethod
    def mute(): return not sys.stdout.isatty()
...
def cmd_web(args: argparse.Namespace) -> None:
    ...
    print(f"{Color.B}🌲{Color.N} BlueDeer {Color.C}Web{Color.N} 仪表盘 → http://{host}:{port}")
...
def cmd_serve(args: argparse.Namespace) -> None:
    ...
    if args.dev:
        ...
        print(f"🦌 BlueDeer 开发模式 — admin 认证已关闭")
...
def cmd_plugins(args: argparse.Namespace) -> None:
    ...
    if args.action == "list":
        ...
        for name in names:
            ...
            print(f"  [{status.get('status','?')}] {m.get('name', name)} v{m.get('version', '?')}")
        ...
        if args.zip:
            ...
            name = args.name
            src = os.path.join("plugins_src", name) if os.path.isdir(os.path.join("plugins_src", name)) else name
        ...
        if not args.url:
            ...
            sys.exit(1)
        ok, msg = repo.install_from_git(args.url, branch=args.branch, target_name=args.name)
        print(msg)
        if ok:
            loaded = asyncio.run(pm.load_one(args.name or os.path.basename(args.url).replace(".git", "")))
            if loaded:
                print(Color.ok("✅ 插件加载成功"))
...
def cmd_agent(args: argparse.Namespace) -> None:
    ...
    if args.action == "list":
        ...
        for a in agents:
            ...
            print(f"  {icon} {a.name:20s} v{a.version:6s} [{a.role:12s}] {a.description[:50]}")
...
def cmd_board(args: argparse.Namespace) -> None:
    ...
    if args.action == "save":
        ...
        stats = h.aggregate()
        print(Color.info(f"📊 任务看板: {stats['total']} 总计, {stats['success']} 成功, {stats['failed']} 失败, {stats['pending']} 待处理"))
        for tid, r in stats["tasks"].items():
            print(f"  [{r['status']}] {tid}" + (f" — {r.get('error','')}" if r.get("error") else ""))
...
def cmd_trace(args: argparse.Namespace) -> None:
    ...
    if filter_by:
        ...
        if len(parts) == 2:
            key, val = parts
            filtered = [l for l in lines if f'"{key}": "{val}"' in l or f'"{key}": "{val}' in l]
...
def cmd_schedule(args: argparse.Namespace) -> None:
    ...
    if args.action == "list":
        ...
        for jid, j in jobs.items():
            ...
            print(f"  {flag} [{jid}] {j.cron} → {j.task_type} {'| ' + j.description if j.description else ''}")
...
def cmd_alert(args: argparse.Namespace) -> None:
    ...
    if args.action in ("status", "list"):
        ...
        print(f"{'ID':20s} {'名称':20s} {'指标':16s} {'条件':12s} {'严重度':10s} {'启用':6s}")
        print("-" * 90)
        for r in rules:
            enabled = "✅" if r["enabled"] else "❌"
            print(f"{r['id']:20s} {r['name']:20s} {r['metric']:16s} "
        ...
        if not events:
            ...
            return
        for e in events:
            ts = __import__("time").strftime("%m-%d %H:%M:%S", __import__("time").localtime(e.get("ts", 0)))
            sev = e.get("severity", "?")
        ...
        if mgr:
            ...
            if summary:
                ...
                for tid, s in summary.items():
                    print(f"  {tid}: 第{s['attempt']}次, 剩余{s['remaining']}次, {s['error'][:60]}")
            else:
                print("\n无活跃重试")
...
def cmd_game(args: argparse.Namespace) -> None:
    ...
    if args.action == "start":
        ...
        try:
            resp = urllib.request.urlopen("http://127.0.0.1:8080/game/api/health", timeout=3)
            d = json.loads(resp.read())
            print(Color.ok(f"🟢 生物圈运行中 ({d.get('employees',0)} 名员工)"))
        except Exception:
            print(Color.err(f"🔴 生物圈未启动"))


def cmd_cleanup(args: argparse.Namespace) -> None:
    """数据清理与维护。"""
    from core.cleanup import run_cleanup, get_storage_stats

    if args.action == "stats":
        stats = get_storage_stats()
        print(Color.info(f"📊 存储用量: {stats['total_str']} ({len(stats['files'])} 个文件)\n"))
...
def cmd_backup(args: argparse.Namespace) -> None:
    """数据备份与恢复。"""
    from core.backup import create_backup, list_backups, restore_backup, delete_backup
...
def main() -> None:
    ...
    serve_p.add_argument("--dev", action="store_true", help="开发模式（关闭 admin 认证）")
    ...
    plugin_p.add_argument("action", choices=["list", "install", "enable", "disable", "search", "install-git", "uninstall"])
    ...
    board_p.add_argument("action", choices=["save", "load", "show"], default="show", nargs="?")
    ...
    trace_p.add_argument("--json", dest="json_output", action="store_true", help="JSON 格式输出")
    ...
    sched_p.add_argument("action", choices=["list", "add", "remove", "enable", "disable"])
    ...
    dag_p.add_argument("action", choices=["list", "add", "remove", "plan", "export", "import"])
    ...
    dag_p.add_argument("--file", "-f", default="bluedeer_dag.json", help="导入/导出文件路径")
    ...
    game_p.add_argument("action", choices=["start", "status"], default="start", nargs="?")

    cleanup_p = sub.add_parser("cleanup", help="数据清理与维护")
    cleanup_p.add_argument("--dry-run", action="store_true", help="试运行（不实际删除）")
    cleanup_p.add_argument("--max-days", type=int, default=14, help="保留天数（默认 14）")
    cleanup_p.add_argument("action", choices=["run", "stats"], default="stats", nargs="?")

    backup_p = sub.add_parser("backup", help="数据备份管理")
    backup_p.add_argument("action", choices=["create", "list", "restore", "delete"], default="list", nargs="?")
    ...
    backup_p.add_argument("--file", "-f", default="", help="备份文件路径（restore/delete 用）")
    ...
    if args.config:
        from core.config import set_config, AppConfig
