...
var NODE_TEMPLATE = {
  agent: function(id){ return { id: id, type: 'agent', data: { agent_id: 'worker1', content: '节点消息内容' } }; },
  tool: function(id){ return { id: id, type: 'tool', data: { skill_id: 'echo', owner_agent_id: 'worker1', params: {} } }; },
  memory_write: function(id){ return { id: id, type: 'memory_write', data: { owner_agent_id: 'worker1', domain: 'global', mem_key: 'note', content: '记忆内容' } }; },
  memory_read: function(id){ return { id: id, type: 'memory_read', data: { owner_agent_id: 'worker1', domain: 'global', mem_key: '' } }; },
  approval: function(id){ return { id: id, type: 'approval', data: {} }; }
};
/* Flowise 式节点库：点按钮把节点模板插入 Definition JSON */
function addNodeToDef(type) {
  var ta = document.getElementById('wf-def');
  if (!ta) return;
  var def;
  try { def = JSON.parse(ta.value.trim() || '{"nodes":[],"edges":[]}'); }
  catch (e) { toast('当前 Definition 不是合法 JSON，请先修复：' + e.message, true); return; }
  if (!def.nodes) def.nodes = [];
  if (!def.edges) def.edges = [];
  var nid = 'n' + (def.nodes.length + 1);
  def.nodes.push(NODE_TEMPLATE[type](nid));
  ta.value = JSON.stringify(def, null, 2);
  toast('已插入节点 ' + nid + '（' + type + '）');
}
