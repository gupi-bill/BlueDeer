...
async def do_resolve_contact(content: str, owner: str | None = None) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        try:
            resp = await client.get(
                f"{_INTERNAL_BASE}/api/email/resolve-contact", params={"name": name}
            )
    ...
    for key, info in contacts.items():
        if info.get("phone"):
            lines.append(
                f"- {info['name']} — phone: {info['phone']} ({info['source']})"
            )
        else:
...
async def do_manage_contact(content: str, owner: str | None = None) -> dict:
    ...
    try:
        ...
        if action == "add":
            ...
            if not name:
                name = (
                    email.split("@")[0]
                    if email
                    else (phones[0] if phones else "Contact")
                )
            ...
            for c in existing:
                ...
                if phones and any(p in (c.get("phones") or []) for p in phones):
                    return {
                        "output": f"{phones[0]} is already a contact ({c.get('name','')}).",
                        "exit_code": 0,
                    }
            ok = await asyncio.to_thread(
                cc._create_contact, name, email, address, phones
            )
            detail = email or ", ".join(phones) or address
            return {
                "output": f"{'Added' if ok else 'Failed to add'} {name} ({detail}).",
        ...
        if action in ("update", "edit"):
            ...
            ok = await asyncio.to_thread(
                cc._update_contact, uid, name, emails, phones, address
            )
            return {
