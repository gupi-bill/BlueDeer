...
async def _call_teacher(
    teacher_model_spec: str, prompt: str, owner: str | None = None
) -> str | None:
    ...
    try:
        url, model, headers = await asyncio.to_thread(
            _resolve_model, teacher_model_spec, owner=owner
        )
...
async def evaluate_turn_llm(
    ...
    url, model, headers = resolve_endpoint(
        "utility", fallback_url=student_endpoint_url, owner=owner
    )
    ...
    try:
        ...
        if response:
            ...
            if cleaned_response == "failure":
                return (
                    "failure",
                    f"LLM evaluation flagged failure: {response.strip()}",
                )
    except Exception as e:
...
async def run_teacher_inline(
    *,
    ...
    try:
        ...
        teacher_url, teacher_model, teacher_headers = await asyncio.to_thread(
            _resolve_model, teacher_spec, owner=owner
        )
    except Exception as e:
