...
HOUSEKEEPING_DEFAULTS = {
    ...
    "email_auto_translate": {
        "name": "Email Auto Translate",
        "schedule": "cron",
        "scheduled_time": None,
        "cron_expression": "0 */2 * * *",
        "ship_paused": True,
        "legacy_names": ["Auto-translate Emails", "Auto Translate Email"],
    },
...
def _normalize_chat_endpoint(url: str) -> str:
    ...
    try:
        ...
        logger.debug(
            "task endpoint normalization failed for %r; using as-is", url, exc_info=True
        )
        return url
...
class TaskScheduler:
        ...
        try:
            ...
            try:
                ...
                groups = (
                    ...
                    .filter(
                        CrewMember.is_default_assistant == True,
                    )
    ...
    def _defer_immediately_due_task(self, task_id: str, *, delay: timedelta):
        ...
        try:
            ...
            logger.debug(
                "Failed to defer cancelled queued task %s", task_id, exc_info=True
            )
    ...
    async def _execute_task_locked(
        ...
        *,
        ...
        try:
            ...
            if is_admin_only_task_action(
                task.task_type, task.action
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info(
                                "Task '%s' interrupted because Odysseus became active",
                                task.name,
                            )
                            if current_task:
                                current_task.cancel()
            ...
            if run.status == "success" and task.then_task_id:
                ...
                chain_task = (
                    db.query(ScheduledTask).filter(ScheduledTask.id == chain_id).first()
                )
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            cfg = _resolve_send_config(
                account_id=account_id or None, owner=task.owner or ""
            )
            ...
            logger.info(
                "Task %s emailed result (recipient_set=%s, %sb)",
                task.id,
                bool(to_addr),
                len(result or ""),
            )
    ...
    def _has_chain_cycle(
        self, db, start_id: str, max_depth: int = 10, owner: str | None = None
    ) -> bool:
    ...
    async def stop_background_tasks_for_foreground(
        self, *, reason: str = "Odysseus became active"
    ) -> int:
    ...
    async def ensure_defaults(self, owner: str):
        ...
        try:
            ...
            if retired_ids:
                db.query(TaskRun).filter(TaskRun.task_id.in_(retired_ids)).delete(
