        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "body": {
                        "type": "string",
                        "description": "For open_email_reply: reply body to pre-fill. Required whenever the user told you what the reply should say. Opens a draft, does not send.",
                    },
                    "colors": {
                        "type": "object",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "start": {
                        "type": "string",
                        "description": "list_events range start (ISO datetime). Use this for month/week requests after resolving the date range; do not pass a loose query string. Prefer start; backend also accepts start_time, start_date, range_start, from, dtstart, since.",
                    },
                    "end": {
                        "type": "string",
                        "description": "list_events range end (ISO datetime). Use this for month/week requests after resolving the date range; defaults to +14 days only when no range is requested. Prefer end; backend also accepts end_time, end_date, range_end, to, dtend, until.",
                    },
                    "event_type": {
                        "type": "string",
                    ...
                    "rrule": {
                        "type": "string",
                        "description": "Recurrence rule in iCalendar RRULE format, e.g. 'FREQ=WEEKLY;BYDAY=MO' for weekly on Monday. Use with create_event or update_event. For update_event, pass an explicit empty string to remove recurrence and make the event single-occurrence.",
                    },
                },
                "required": ["action"],
            },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "cmd": {
                        "type": "string",
                        "description": "Full serve command (e.g. 'vllm serve <repo> --port 8000 --tp 2', 'python3 -m sglang.launch_server --model-path <repo> --port 30000', for MLX image models: 'python3 scripts/mlx_image_server.py --model <repo> --port 8100', or for non-MLX image models: 'python3 scripts/diffusion_server.py --model <repo> --port 8100')",
                    },
                    "host": {
                        "type": "string",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "host": {
                        "type": "string",
                        "description": "Friendly Cookbook server name (e.g. 'workstation', 'gpu-box') or raw remote host (e.g. 'user@gpu-box'). Omit for local.",
                    },
                    "model_dir": {
                        "type": "string",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "email": {
                        "type": "string",
                        "description": "Single email address (convenience for add, or the primary email for update). Optional when phone or address is provided.",
                    },
                    "emails": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Full list of email addresses (first is primary).",
                    },
                    "phones": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Full list of phone numbers. Valid for add/update.",
                    },
                    "address": {
                        "type": "string",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "folder": {
                        "type": "string",
                        "description": "IMAP folder to scan (default: INBOX)",
                    },
                    "limit": {
                        "type": "integer",
                        "description": "Maximum candidates to return (default: 25)",
                    },
                    "max_scan": {
                        "type": "integer",
                        "description": "How many newest emails to inspect (default: 150)",
                    },
                    "account": {
                        "type": "string",
                        "description": "Optional account name/email/id from list_email_accounts",
                    },
                },
            },
        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "uid": {
                        "type": "string",
                        "description": "Email UID from scan_email_unsubscribes/list_emails",
                    },
                    "folder": {
                        "type": "string",
                        "description": "IMAP folder (default: INBOX)",
                    ...
                    "method_index": {
                        "type": "integer",
                        "description": "Method index from scan_email_unsubscribes (default: 0)",
                    },
                    "allow_web": {
                        "type": "boolean",
                        "description": "Return browser/web instructions when selected method is URL",
                    },
                    "account": {
                        "type": "string",
                        "description": "Optional account name/email/id from list_email_accounts",
...
def _repair_document_function_args(tool_type: str, arguments: str) -> dict | None:
...
def function_call_to_tool_block(name: str, arguments: str) -> ToolBlock | None:
    ...
    try:
        ...
        if args is not None:
            logger.warning(
                f"Repaired malformed document function call arguments for {name}"
            )
        else:
            logger.error(
                f"Failed to parse function call arguments for {name}: {arguments}"
            )
            return None
    ...
    if not isinstance(args, dict):
        if tool_type.startswith("mcp__email__") or name in BUILTIN_EMAIL_TOOLS:
            logger.warning(
                f"Non-object email function call arguments for {name}: {args!r}; rejecting"
            )
            return None
        logger.warning(
    ...
    if required_args and not any(
        str(args.get(key) or "").strip() for key in required_args
    ):
        logger.warning(
            f"Rejecting empty required arguments for function call {name}: {args!r}"
        )
        return None
