...
def _configure_security_middleware(application, allowed_hosts, allowed_origins):
    ...
    if application.middleware_stack is not None:
        raise RuntimeError(
            "security middleware must be configured before the app starts serving"
        )
...
def _update_progress(
    request_id: str, step: int, total_steps: int | None = None, status: str = "running"
):
    if not request_id:
        return
...
def _load_omnigen2_pipeline(
    model_path: str, torch_dtype, target_device: str, use_offload: bool
) -> bool:
...
def load_model():
    ...
    if target_device != "cuda" and use_offload:
        logger.warning(
            "CPU offload requested but %s is the active device; using direct device placement instead",
            target_device,
        )
        use_offload = False

    logger.info(
        f"Loading model from {model_path} (dtype={_args.dtype}, offload={use_offload}, device={target_device})..."
    )
    ...
    loaded = False
    if (
        cls_name_from_index == "OmniGen2Pipeline"
        or "omnigen2" in str(model_path).lower()
    ):
        loaded = _load_omnigen2_pipeline(
            model_path, torch_dtype, target_device, use_offload
        )
    ...
    def _load_pipe(cls, name):
        ...
        if not use_offload:
            logger.error(
                f"{name} could not be placed on {target_device}. On CUDA, try --cpu-offload; on Apple Silicon try a smaller model or lower resolution."
            )
            return False
    ...
    if not loaded:
        ...
        if single_file:
            ...
            explicit_configs = [
                c.strip()
                for c in str(_args.single_file_config or "")
                .replace("\n", ",")
                .split(",")
                if c.strip()
            ]
                ...
                for config in configs:
                    try:
                        ...
                        if use_offload and _can_cpu_offload(target_device):
                            ...
                            _pipe = _pipe.to(target_device)
                            logger.info(
                                f"Loaded as {cls_name} (single file, config={config}) on {target_device}"
                            )
                        loaded = True
                        break
                    except Exception as e:
...
def generate_image(req: ImageRequest):
    ...
    request_id = _start_progress(
        req.request_id, steps * max(1, int(req.n or 1)), req.prompt, "generation"
    )
    ...
    try:
        for image_index in range(req.n):
            ...
            negative_prompt = (
                ...
                if _pipeline_accepts_arg("negative_prompt")
                else None
            )
            if _is_inpaint_pipe:
            ...
            _update_progress(
                request_id, progress_offset + steps, steps * max(1, int(req.n or 1))
            )
            img = result.images[0]
            images.append(img)
    except Exception as e:
...
async def edit_image(
    ...
    request_id = _start_progress(
        request_id, steps * max(1, min(int(n or 1), 4)), prompt, "edit"
    )
    raw = await image.read()
    init_image = PILImage.open(io.BytesIO(raw)).convert("RGB")
    if width > 0 and height > 0:
        init_image = ImageOps.fit(
            init_image, (width, height), method=PILImage.LANCZOS, centering=(0.5, 0.5)
        )
            ...
            if accepts_input_images and not accepts_image:
                ...
                kwargs = _pipeline_call_kwargs(
                    ...
                    max_input_image_side_length=(
                        max(width, height) if width > 0 and height > 0 else None
                    ),
                )
            else:
                kwargs = _pipeline_call_kwargs(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--negative-prompt",
        default="",
        help="Default negative prompt for pipelines that support it",
    )
    parser.add_argument(
        "--single-file-config",
        default="",
        help="Base Diffusers repo/path for single-file checkpoints that need missing components. Comma-separated values are tried in order.",
    )
