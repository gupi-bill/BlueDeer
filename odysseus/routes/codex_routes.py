...
def setup_opencode_routes() -> APIRouter:
    """Serve the OpenCode skill bundle.

    OpenCode (opencode.ai) is a terminal coding agent in the same family as
    Claude Code and Codex: at runtime it uses the shared scope-gated
    ``/api/codex/*`` endpoints. This router only exists to deliver the skill
    zip via ``/api/opencode/plugin.zip`` so the user-facing setup commands
    stay in the OpenCode namespace, mirroring ``setup_claude_routes``.
    """
    ...
    def plugin_zip(request: Request):
        require_authenticated_request(request)
        # Only ship the skills/ subtree so extracting at
        # ~/.config/opencode/skills/ drops the skill directly under that dir
        # (~/.config/opencode/skills/odysseus/SKILL.md) — OpenCode auto-loads
        # skills from its skills directory, unlike Codex which uses a plugin
        # descriptor that tolerates an extra wrapper folder.
        skills_root = Path(__file__).resolve().parent.parent / "integrations" / "opencode" / "skills"
        if not skills_root.exists():
            raise HTTPException(404, "OpenCode skill bundle not found")
        ...
        buf.seek(0)
        headers = {"Content-Disposition": 'attachment; filename="odysseus-opencode-skill.zip"'}
        return StreamingResponse(buf, media_type="application/zip", headers=headers)
