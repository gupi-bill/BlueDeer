    ...
    if channel == "ntfy":
        try:
            ...
            if intg:
                ...
                _block = (
                    _os.getenv("REMINDER_WEBHOOK_BLOCK_PRIVATE_IPS", "false").lower()
                    == "true"
                )
    ...
    if (
        ...
        try:
            ...
            sent_channel = (
                "email"
                if email_sent
                else "ntfy" if ntfy_sent else "webhook" if webhook_sent else "browser"
            )
...
def setup_note_routes(task_scheduler=None, upload_handler=None):
    ...
    def _reserve_note_uploads(owner: str | None, *values) -> None:
    ...
    async def reorder_notes(request: Request):
        ...
        try:
            for i, nid in enumerate(ids):
                ...
                if user is not None:
                    if _allow_null:
                        q = q.filter((Note.owner == user) | (Note.owner == None))
                    else:
                        q = q.filter(Note.owner == user)
                note = q.first()
                if note:
