...
def _append_mlx_image_server_script(runner_lines: list[str]) -> None:
    ...
    script_path = (
        Path(__file__).resolve().parents[1] / "scripts" / "mlx_image_server.py"
    )
    try:
        ...
        runner_lines.append(
            'echo "ERROR: Odysseus could not prepare the MLX image server helper."'
        )
...
def _append_openai_port_preflight_lines(
    lines: list[str], *, cmd: str, expected_model: str
) -> None:
    ...
    lines.append(
        '  python3 - "$ODYSSEUS_SERVE_PORT" "$ODYSSEUS_EXPECTED_MODEL" <<\'PY\''
    )
    ...
    lines.append(
        '  if [ "$_ody_port_ec" -ne 0 ]; then ODYSSEUS_PREFLIGHT_EXIT="$_ody_port_ec"; fi'
    )
    lines.append("fi")
...
def setup_cookbook_routes() -> APIRouter:
    ...
    def _diagnose_serve_output(text: str) -> dict | None:
        ...
        patterns = [
            ...
            (
                ...
                [
                    {
                        "label": "repair sglang-kernel in this Python environment",
                        "op": "dependency",
                        "package": "sglang-kernel",
                    },
                    {
                        "label": "if libnvrtc is still missing, install the matching CUDA/NVRTC runtime on this host",
                        "op": "manual",
                    },
                ],
            ),
            ...
            (
                ...
                [
                    {
                        "label": "install mlx-lm in Cookbook Dependencies",
                        "op": "dependency",
                        "package": "mlx-lm",
                    }
                ],
            ...
            (
                ...
                [
                    {
                        "label": "update Diffusers image dependencies",
                        "op": "dependency",
                        "package": "diffusers transformers accelerate",
                    }
                ],
            ),
            (
                ...
                [
                    {
                        "label": "relaunch from the cached local Hugging Face snapshot path on this Mac",
                        "op": "manual",
                    },
                    {
                        "label": "Odysseus now rewrites MLX repo-id launches to a cached snapshot when one exists",
                        "op": "manual",
                    },
                ],
            ),
            ...
            (
                ...
                [
                    {
                        "label": "install Diffusers image deps in Cookbook Dependencies",
                        "op": "dependency",
                        "package": "diffusers[torch] torchvision accelerate scipy python-multipart",
                    }
                ],
            ),
            (
        ...
        if re.search(
            r"Traceback \(most recent call last\)", tail, re.IGNORECASE
        ) and not re.search(
            r"Application startup complete|GET /v1/|Uvicorn running on",
            tail,
            re.IGNORECASE,
        ):
            return {
    ...
    def _normalize_deepseek_v4_sglang_cmd(cmd: str) -> str:
        ...
        try:
            mem_fraction_num = (
                float(mem_fraction) if mem_fraction not in (None, "") else None
            )
    ...
    def _ssh_host_key_changed(stderr_txt: str) -> bool:
        ...
        return (
            ...
            and "Offending" in text
        )

    async def _repair_cookbook_known_host(
        host: str, ssh_port: str | None = None
    ) -> tuple[bool, str]:
        ...
        return (
            True,
            "\n".join(chunk for chunk in removed_chunks if chunk)
            or "known_hosts refreshed",
        )
    ...
    def _server_env_prefix_for_download(remote_host: str | None) -> str | None:
        ...
        for server in servers:
            if (
                ...
                and (server.get("host") or "").strip() == remote_host
            ):
                selected = server
                break
        if not selected:
        ...
        if env == "venv" or (
            env in {"", "none"}
            and re.search(
                r"(?:^|/)(?:\.?venv|env)(?:/|$)|/bin/activate$", env_path, re.IGNORECASE
            )
        ):
            activate = (
                env_path
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        if remote and is_windows:
            ...
            runner_lines.append(
                'ODYSSEUS_PY="$(command -v python3 || command -v python || true)"'
            )
            runner_lines.append(
                'if [ -z "$ODYSSEUS_PY" ]; then echo "ERROR: python3/python not found on this server."; exit 127; fi'
            )
            ...
            if is_ollama_download:
                ...
                runner_lines.append(
                    f"command -v hf >/dev/null 2>&1 || command -v huggingface-cli >/dev/null 2>&1 || {hf_hub_install}"
                )
                runner_lines.append("hash -r 2>/dev/null || true")
                runner_lines.append(
                    'ODYSSEUS_HF_CLI="$(command -v hf || command -v huggingface-cli || true)"'
                )
                runner_lines.append(
                    'if [ -z "$ODYSSEUS_HF_CLI" ]; then echo "ERROR: HF CLI not found after installing huggingface_hub."; exit 127; fi'
                )
                if req.disable_hf_transfer:
                    ...
                    runner_lines.append(
                        f"\"$ODYSSEUS_PY\" -c 'import hf_transfer' 2>/dev/null || {hf_transfer_install}"
                    )
                    runner_lines.append(
                        "\"$ODYSSEUS_PY\" -c 'import hf_transfer' 2>/dev/null && export HF_HUB_ENABLE_HF_TRANSFER=1"
                    )
            ...
            if is_ollama_download:
                ...
                runner_lines.append(
                    f'  "$ODYSSEUS_HF_CLI" {hf_download_args} < /dev/null'
                )
    ...
    async def model_cached(
        ...
        if model_dir:
            for d in model_dir.split(","):
                ...
                if d:
                    if d.startswith(
                        ("home/", "mnt/", "media/", "data/", "opt/", "srv/", "var/")
                    ):
                        d = "/" + d
                    model_dirs.append(d)
        ...
        if host and returncode != 0 and _ssh_host_key_changed(stderr_txt):
            ...
            if ok:
                logger.info(
                    "Repaired Cookbook known_hosts for %s after host-key-change scan failure",
                    host,
                )
                (stdout_b, stderr_b), returncode = await _run_cached_scan_once()
                stderr_txt = stderr_b.decode(errors="replace").strip()
                ...
                logger.warning(
                    "Failed to repair Cookbook known_hosts for %s: %s",
                    host,
                    detail[:300],
                )
        if returncode != 0:
            ...
            logger.warning(
                f"Cached model scan failed host={host or 'local'} rc={returncode}: {msg[:500]}"
            )
        ...
        if remote:
            ...
            capture_cmd = ssh_args + [
                ...
                _remote_tmux_command(
                    "capture-pane", "-t", session_id, "-p", "-S", "-2000"
                ),
            ]
        else:
            capture_cmd = [
    ...
    def _auto_register_llm_endpoint(
        ...
        if is_mlx_deepseek_v4 and short_name:
            ...
            if remote_home:
                mlx_shim_model_id = (
                    f"{remote_home}/.cache/odysseus/mlx-shims/{short_name}"
                )
        ...
        pinned_models = (
            ...
            else ([req.repo_id] if req.repo_id else [])
        )
        ...
        try:
            ...
            if existing:
                ...
                if pinned_models:
                    ...
                    existing.pinned_models = (
                        json.dumps(merged_pinned) if merged_pinned else None
                    )
            ...
            try:
                if mlx_shim_model_id:
                    ...
                    db.commit()
                    logger.info(
                        f"Auto-register: pinned MLX DeepSeek-V4 shim model @ {base_url}"
                    )
                else:
                    import json as _json2
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if (
                ...
                runner_lines.append(f"ODYSSEUS_SERVE_CMD='{_bash_squote(req.cmd)}'")
                runner_lines.append(
                    'ODYSSEUS_SGLANG_CMD_PY="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                runner_lines.append("import shlex, sys")
                runner_lines.append("parts = shlex.split(sys.argv[1])")
                runner_lines.append('py = "python3"')
                ...
                runner_lines.append(
                    '    if part.endswith("/bin/python") or part.endswith("/bin/python3") or "/bin/python3." in part:'
                )
                ...
                runner_lines.append(')"')
                runner_lines.append(
                    'if ! "$ODYSSEUS_SGLANG_CMD_PY" -c "import sglang" &>/dev/null; then'
                )
                runner_lines.append("  if ! command -v sglang &>/dev/null; then")
                runner_lines.append('    echo "ERROR: SGLang is not installed."')
                runner_lines.append("  else")
                runner_lines.append(
                    '    echo "ERROR: SGLang is installed but failed to import in the launch Python."'
                )
                runner_lines.append("  fi")
                runner_lines.append(
                    '  ODYSSEUS_SGLANG_IMPORT_ERROR="$("$ODYSSEUS_SGLANG_CMD_PY" -c "import sglang" 2>&1)"'
                )
                ...
                runner_lines.append(f"ODYSSEUS_SERVE_CMD='{_bash_squote(req.cmd)}'")
                runner_lines.append(
                    'ODYSSEUS_MLX_CMD_PY="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                runner_lines.append("import shlex, sys")
                runner_lines.append("parts = shlex.split(sys.argv[1])")
                runner_lines.append('py = "python3"')
                ...
                runner_lines.append(')"')
                runner_lines.append(
                    'if ! ODYSSEUS_MLX_IMPORT_ERROR="$("$ODYSSEUS_MLX_CMD_PY" -c "import mlx_lm" 2>&1)"; then'
                )
                runner_lines.append(
                    '  echo "ERROR: MLX LM is not installed in the launch Python: $ODYSSEUS_MLX_CMD_PY"'
                )
                ...
                runner_lines.append('if [ -z "$ODYSSEUS_PREFLIGHT_EXIT" ]; then')
                runner_lines.append(
                    '  ODYSSEUS_SERVE_CMD="$("$ODYSSEUS_MLX_CMD_PY" - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                ...
                runner_lines.append(
                    'if "/" in model and not model.startswith("/") and model.startswith("mlx-community/"):'
                )
                ...
                runner_lines.append(
                    '            if not os.path.exists(os.path.join(path, "config.json")): continue'
                )
                ...
                runner_lines.append(
                    '        print("[odysseus] MLX using cached snapshot:", best, file=sys.stderr)'
                )
                ...
                runner_lines.append(
                    "                utils_path = Path(mlx_utils.__file__)"
                )
                runner_lines.append(
                    "                utils_text = utils_path.read_text()"
                )
                runner_lines.append(
                    '                utils_needle = \'        def class_predicate(p, m):\\n            # Handle custom per layer quantizations\\n            if p in config["quantization"]:\\n                return config["quantization"][p]\\n            if not hasattr(m, "to_quantized"):\\n                return False\\n            return f"{p}.scales" in weights\\n\''
                )
                runner_lines.append(
                    '                utils_repl = \'        def class_predicate(p, m):\\n            # Odysseus: DeepSeek-V4 MXFP4 switch layers may already be quantized.\\n            if type(m).__name__ == "QuantizedSwitchLinear":\\n                return False\\n            # Handle custom per layer quantizations\\n            if p in config["quantization"]:\\n                return config["quantization"][p]\\n            if not hasattr(m, "to_quantized"):\\n                return False\\n            return f"{p}.scales" in weights\\n\''
                )
                runner_lines.append(
                    "                if utils_repl not in utils_text and utils_needle in utils_text:"
                )
                runner_lines.append(
                    '                    bak = utils_path.with_suffix(utils_path.suffix + ".odysseus_bak")'
                )
                runner_lines.append(
                    "                    if not bak.exists(): bak.write_text(utils_text)"
                )
                runner_lines.append(
                    "                    utils_path.write_text(utils_text.replace(utils_needle, utils_repl))"
                )
                runner_lines.append(
                    '                    print("[odysseus] Patched MLX-LM QuantizedSwitchLinear double-quantization guard.", file=sys.stderr)'
                )
                runner_lines.append("                dsv4_path = Path(dsv4.__file__)")
                runner_lines.append("                dsv4_text = dsv4_path.read_text()")
                runner_lines.append(
                    '                dsv4_needle = \'            for sub in ("attn", "ffn"):\\n                for p in ("fn", "base", "scale"):\\n                    nk = nk.replace(f".hc_{sub}_{p}", f".hc_{sub}.{p}")\\n            for wo, wn in w_remap.items():\\n\''
                )
                runner_lines.append(
                    '                dsv4_repl = \'            for sub in ("attn", "ffn"):\\n                for p in ("fn", "base", "scale"):\\n                    nk = nk.replace(f".hc_{sub}_{p}", f".hc_{sub}.{p}")\\n            # Odysseus: normalize alternate hyper-connection key aliases.\\n            nk = nk.replace(".attn_hc.", ".hc_attn.")\\n            nk = nk.replace(".ffn_hc.", ".hc_ffn.")\\n            for wo, wn in w_remap.items():\\n\''
                )
                runner_lines.append(
                    "                if dsv4_repl not in dsv4_text and dsv4_needle in dsv4_text:"
                )
                runner_lines.append(
                    '                    bak = dsv4_path.with_suffix(dsv4_path.suffix + ".odysseus_bak")'
                )
                runner_lines.append(
                    "                    if not bak.exists(): bak.write_text(dsv4_text)"
                )
                runner_lines.append(
                    "                    dsv4_path.write_text(dsv4_text.replace(dsv4_needle, dsv4_repl))"
                )
                runner_lines.append(
                    '                    print("[odysseus] Patched MLX-LM DeepSeek-V4 hyper-connection key aliases.", file=sys.stderr)'
                )
                runner_lines.append("            except Exception as e:")
                runner_lines.append(
                    '                print("[odysseus] WARNING: failed to apply MLX DeepSeek-V4 compatibility patch:", e, file=sys.stderr)'
                )
                runner_lines.append("            try:")
                runner_lines.append("                src = Path(best)")
                runner_lines.append(
                    '                shim = Path.home() / ".cache" / "odysseus" / "mlx-shims" / src.name'
                )
                runner_lines.append("                if len(src.name) > 20:")
                runner_lines.append(
                    '                    shim = Path.home() / ".cache" / "odysseus" / "mlx-shims" / model.split("/")[-1]'
                )
                runner_lines.append(
                ...
                runner_lines.append(
                    '                    if tc.get("tool_parser_type") == "deepseek_v4":'
                )
                runner_lines.append(
                    '                        tc.pop("tool_parser_type", None)'
                )
                runner_lines.append(
                    '                    (shim / "tokenizer_config.json").write_text(json.dumps(tc, indent=2, ensure_ascii=False))'
                )
                runner_lines.append("                    launch_model = str(shim)")
                runner_lines.append(
                    '                    print("[odysseus] MLX DeepSeek-V4 using sanitized shim:", launch_model, file=sys.stderr)'
                )
                runner_lines.append("            except Exception as e:")
                runner_lines.append(
                    '                print("[odysseus] WARNING: failed to create MLX DeepSeek-V4 shim:", e, file=sys.stderr)'
                )
                runner_lines.append("        parts[i + 1] = launch_model")
                runner_lines.append("    else:")
                runner_lines.append(
                    '        print("[odysseus] MLX cached snapshot not found for:", model, file=sys.stderr)'
                )
                runner_lines.append("print(shlex.join(parts))")
                runner_lines.append("PY")
                runner_lines.append(')"')
                ...
                "scripts/mlx_image_server.py" in req.cmd
                or ".mlx_image_server.py" in req.cmd
            ):
                ...
                runner_lines.append(f"ODYSSEUS_SERVE_CMD='{_bash_squote(req.cmd)}'")
                runner_lines.append(
                    'ODYSSEUS_MLX_IMAGE_CMD_PY="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                runner_lines.append("import shlex, sys")
                runner_lines.append("parts = shlex.split(sys.argv[1])")
                runner_lines.append('py = "python3"')
                ...
                runner_lines.append(')"')
                runner_lines.append(
                    'ODYSSEUS_MLX_IMAGE_BIN_DIR="$(dirname "$ODYSSEUS_MLX_IMAGE_CMD_PY" 2>/dev/null || true)"'
                )
                runner_lines.append(
                    'if [ -n "$ODYSSEUS_MLX_IMAGE_BIN_DIR" ]; then export PATH="$ODYSSEUS_MLX_IMAGE_BIN_DIR:$PATH"; fi'
                )
                runner_lines.append(
                    'if ! "$ODYSSEUS_MLX_IMAGE_CMD_PY" -c "import fastapi, uvicorn, multipart" >/dev/null 2>&1; then'
                )
                runner_lines.append(
                    '  echo "ERROR: MLX image serving requires FastAPI + uvicorn + python-multipart in the launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY. Install the MLX image dependencies in Cookbook Dependencies."'
                )
                runner_lines.append("  ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("fi")
                runner_lines.append(
                    'ODYSSEUS_MLX_IMAGE_MODEL="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                runner_lines.append("import shlex, sys")
                runner_lines.append("parts = shlex.split(sys.argv[1])")
                ...
                runner_lines.append(')"')
                runner_lines.append(
                    'if printf "%s" "$ODYSSEUS_MLX_IMAGE_MODEL" | grep -qi hidream; then'
                )
                runner_lines.append(
                    '  if ! "$ODYSSEUS_MLX_IMAGE_CMD_PY" -c "import mlx, mlx_vlm, transformers, huggingface_hub, safetensors, numpy, PIL" >/dev/null 2>&1; then'
                )
                runner_lines.append(
                    '    echo "ERROR: HiDream MLX serving needs the model requirements in the launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."'
                )
                runner_lines.append(
                    "    echo \"Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U fastapi uvicorn python-multipart mlx mlx-vlm 'transformers>=4.57.0,<6.0' huggingface_hub safetensors numpy pillow tqdm sentencepiece hf_transfer\""
                )
                runner_lines.append("    ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("  fi")
                runner_lines.append(
                    'elif printf "%s" "$ODYSSEUS_MLX_IMAGE_MODEL" | grep -qi boogu; then'
                )
                runner_lines.append(
                    '  if ! "$ODYSSEUS_MLX_IMAGE_CMD_PY" -c "import boogu_image_mlx, mlx, huggingface_hub, safetensors, numpy, PIL" >/dev/null 2>&1; then'
                )
                runner_lines.append(
                    '    echo "ERROR: Boogu MLX serving needs boogu-image-mlx in the launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."'
                )
                runner_lines.append(
                    '    echo "Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U git+https://github.com/xocialize/boogu-image-mlx.git fastapi uvicorn python-multipart pillow"'
                )
                runner_lines.append("    ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("  fi")
                runner_lines.append(
                    'elif printf "%s" "$ODYSSEUS_MLX_IMAGE_MODEL" | grep -Eqi "ddcolor"; then'
                )
                runner_lines.append(
                    '  if ! "$ODYSSEUS_MLX_IMAGE_CMD_PY" -c "import PIL" >/dev/null 2>&1; then'
                )
                runner_lines.append(
                    '    echo "ERROR: DDColor MLX serving needs Pillow in the launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."'
                )
                runner_lines.append(
                    '    echo "Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U fastapi uvicorn python-multipart pillow huggingface_hub"'
                )
                runner_lines.append("    ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("  fi")
                runner_lines.append(
                    "  if ! command -v odysseus-mlx-colorize >/dev/null 2>&1 && ! command -v mlx-ddcolor-serve >/dev/null 2>&1; then"
                )
                runner_lines.append(
                    '    echo "ERROR: DDColor MLX serving requires the Odysseus mlx-ddcolor-swift bridge on PATH: odysseus-mlx-colorize or mlx-ddcolor-serve."'
                )
                runner_lines.append(
                    '    echo "Build it from swift/odysseus-mlx-image-bridge in Cookbook Dependencies."'
                )
                runner_lines.append("    ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("  fi")
                runner_lines.append(
                    '  ODYSSEUS_DDCOLOR_BIN="$(command -v odysseus-mlx-colorize 2>/dev/null || command -v mlx-ddcolor-serve 2>/dev/null || true)"'
                )
                runner_lines.append('  if [ -n "$ODYSSEUS_DDCOLOR_BIN" ]; then')
                runner_lines.append(
                    '    ODYSSEUS_DDCOLOR_DIR="$(dirname "$ODYSSEUS_DDCOLOR_BIN")"'
                )
                runner_lines.append(
                    '    if [ ! -f "$ODYSSEUS_DDCOLOR_DIR/mlx.metallib" ] && [ ! -f "$ODYSSEUS_DDCOLOR_DIR/default.metallib" ]; then'
                )
                runner_lines.append(
                    '      echo "ERROR: DDColor MLX serving found the Swift runner, but mlx.metallib/default.metallib is missing next to it."'
                )
                runner_lines.append(
                    '      echo "Run the DDColor MLX image editing dependency install again; it copies mlx.metallib from the launch Python MLX package."'
                )
                runner_lines.append("      ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("    fi")
                runner_lines.append("  fi")
                runner_lines.append(
                    'elif printf "%s" "$ODYSSEUS_MLX_IMAGE_MODEL" | grep -Eqi "mi-gan|migan|lama"; then'
                )
                runner_lines.append(
                    '  if ! "$ODYSSEUS_MLX_IMAGE_CMD_PY" -c "import PIL" >/dev/null 2>&1; then'
                )
                runner_lines.append(
                    '    echo "ERROR: LaMa / MI-GAN MLX serving needs Pillow in the launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."'
                )
                runner_lines.append(
                    '    echo "Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U fastapi uvicorn python-multipart pillow huggingface_hub"'
                )
                ...
                runner_lines.append("  fi")
                runner_lines.append(
                    "  if ! command -v odysseus-mlx-inpaint >/dev/null 2>&1 && ! command -v mlx-lama-serve >/dev/null 2>&1; then"
                )
                runner_lines.append(
                    '    echo "ERROR: LaMa / MI-GAN MLX serving requires the Odysseus mlx-lama-swift bridge on PATH: odysseus-mlx-inpaint or mlx-lama-serve."'
                )
                runner_lines.append(
                    '    echo "Build it from swift/odysseus-mlx-image-bridge in Cookbook Dependencies."'
                )
                ...
                runner_lines.append("  fi")
                runner_lines.append(
                    '  ODYSSEUS_INPAINT_BIN="$(command -v odysseus-mlx-inpaint 2>/dev/null || command -v mlx-lama-serve 2>/dev/null || true)"'
                )
                runner_lines.append('  if [ -n "$ODYSSEUS_INPAINT_BIN" ]; then')
                runner_lines.append(
                    '    ODYSSEUS_INPAINT_DIR="$(dirname "$ODYSSEUS_INPAINT_BIN")"'
                )
                runner_lines.append(
                    '    if [ ! -f "$ODYSSEUS_INPAINT_DIR/mlx.metallib" ] && [ ! -f "$ODYSSEUS_INPAINT_DIR/default.metallib" ]; then'
                )
                runner_lines.append(
                    '      echo "ERROR: LaMa / MI-GAN MLX serving found the Swift runner, but mlx.metallib/default.metallib is missing next to it."'
                )
                runner_lines.append(
                    '      echo "Run the LaMa / MI-GAN MLX image editing dependency install again; it copies mlx.metallib from the launch Python MLX package."'
                )
                runner_lines.append("      ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("    fi")
                runner_lines.append("  fi")
                runner_lines.append(
                    "elif ! command -v mflux-generate >/dev/null 2>&1 && ! command -v mflux-generate-qwen >/dev/null 2>&1; then"
                )
                runner_lines.append(
                    '  echo "ERROR: mflux-compatible MLX image serving requires mflux-generate or mflux-generate-qwen in PATH for launch Python: $ODYSSEUS_MLX_IMAGE_CMD_PY."'
                )
                runner_lines.append(
                    '  echo "Install with: $ODYSSEUS_MLX_IMAGE_CMD_PY -m pip install -U mflux fastapi uvicorn python-multipart"'
                )
                runner_lines.append("  ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("fi")
            elif (
                ...
                runner_lines.append(f"ODYSSEUS_SERVE_CMD='{_bash_squote(req.cmd)}'")
                runner_lines.append(
                    'ODYSSEUS_DIFFUSION_CMD_PY="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                runner_lines.append("import shlex, sys")
                runner_lines.append("parts = shlex.split(sys.argv[1])")
                runner_lines.append('py = "python3"')
                ...
                runner_lines.append(')"')
                runner_lines.append(
                    'if ! ODYSSEUS_DIFFUSION_IMPORT_ERROR="$("$ODYSSEUS_DIFFUSION_CMD_PY" -c "import torch, torchvision, diffusers" 2>&1)"; then'
                )
                runner_lines.append(
                    '  echo "ERROR: Diffusion serving requires PyTorch + Torchvision + diffusers in the launch Python: $ODYSSEUS_DIFFUSION_CMD_PY."'
                )
                runner_lines.append(
            ...
            if not handled_ollama_serve and not handled_ollama_sidecar_probe:
                ...
                if "vllm serve" in req.cmd or "mlx_lm.server" in req.cmd:
                    ...
                    if not is_windows and (req.platform or "").lower() in {
                        "darwin",
                        "macos",
                    }:
                        req.cmd = _pip_install_command_without_break_system_packages(
                            req.cmd
                        )
        ...
        endpoint_id = None
        is_image_endpoint = (
            "diffusion_server.py" in req.cmd or "mlx_image_server.py" in req.cmd
        )
    ...
    async def _probe_apple_unified_memory(
        host: str | None, ssh_port: str | None
    ) -> dict | None:
        ...
        free_mb = max(
            0, min(total_mb, int((page_size * available_pages) / (1024 * 1024)))
        )
    ...
    async def get_cookbook_state(request: Request):
        ...
        try:
            mtime = (
                _cookbook_state_path.stat().st_mtime
        ...
        if (
            cached is not None
            and _state_get_cache.get("mtime") == mtime
            and now - float(_state_get_cache.get("ts") or 0) < 1.5
        ):
            return cached
        if _cookbook_state_path.exists():
            try:
                ...
                _state_get_cache.update(
                    {"ts": now, "mtime": mtime, "value": client_state}
                )
                return client_state
            except Exception:
            ...
            for line in (ls.stdout or "").splitlines():
                ...
                if sid.startswith("cookbook-"):
                    ...
                    try:
                        ...
                        script_text = ""
                    m_repo = re.search(
                        r"repo_id\s*=\s*['\"]([^'\"]+/[^'\"]+)['\"]", script_text
                    )
                    if not m_repo:
                        m_repo = re.search(
                            r"snapshot_download\(\s*repo_id\s*=\s*['\"]([^'\"]+/[^'\"]+)['\"]",
                            script_text,
                        )
                    if not m_repo:
                        m_repo = re.search(
                    ...
                    if not m_repo and pane:
                        m_cache = re.search(
                            r"models--([A-Za-z0-9_.-]+)--([A-Za-z0-9_.-]+)", pane
                        )
                        if m_cache:
                    ...
                    tasks.append(
                        {
                            ...
                            "type": "download",
                            "status": (
                                "completed" if adopted_download_done else "running"
                            ),
                            "progress": (
                                "Download complete" if adopted_download_done else ""
                            ),
                            "output": (
                                pane
                                or f"Auto-adopted from orphan tmux download session on {host}."
                            )[-5000:],
                            "ts": int(_t2.time() * 1000),
                            "completedAt": (
                                int(_t2.time() * 1000)
                                if adopted_download_done
                                else None
                            ),
                            "payload": {
                                "repo_id": repo_id,
                    ...
                    adopted_any = True
                    logger.info(
                        f"auto-adopted orphan download tmux session {sid!r} on {host}"
                    )
                    continue
    ...
    def _cookbook_tasks_status_sync():
        ...
        for task in tasks:
            ...
            if task_platform == "windows" and remote:
                ...
                check_cmd = ssh_base + [
                    remote,
                    _remote_tmux_command("has-session", "-t", session_id),
                ]
                ...
                capture_cmd = ssh_base + [
                    remote,
                    _remote_tmux_command(
                        "capture-pane", "-t", session_id, "-p", "-S", "-500"
                    ),
                ]
            elif IS_WINDOWS:
            ...
            progress_text = ""
            full_snapshot = (
                (task.get("output") or "")[-12000:] if task_type == "serve" else ""
            )
            ...
            if local_win_task:
                ...
                _persisted_serve_ready = (
                    ...
                    and _parse_serve_phase(full_snapshot, task_type).get("status")
                    == "ready"
                )
                if (
                    _task_status
                    ...
                    and not _persisted_serve_ready
                ):
                    is_alive = False
            ...
            results.append(
                {
                    ...
                    "status": status,
                    "progress": (
                        serve_phase if task_type == "serve" else progress_text[:120]
                    ),
                    "phase": serve_phase,
                    "diagnosis": diagnosis,
