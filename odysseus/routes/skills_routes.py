...
_VERDICT_PROSE_RE = re.compile(
    r'verdict["\'\s:]*["\']?(pass|needs_work|fail|inconclusive)', re.IGNORECASE
)
...
async def _eval_skill_run(
    ...
    def _parse(raw: str):
        ...
        text = _re.sub(
            r"<think(?:ing)?>[\s\S]*$", "", text, flags=_re.IGNORECASE
        ).strip()
...
async def _eval_skill_necessity(
    ...
    text = _re.sub(r"<think(?:ing)?>[\s\S]*$", "", text, flags=_re.IGNORECASE).strip()
    data = None
    a, b = text.find("{"), text.rfind("}")
    if a >= 0 and b > a:
        frag = text[a : b + 1]
...
async def _improve_skill_md(
    ...
    text = _re.sub(r"<think(?:ing)?>[\s\S]*$", "", text, flags=_re.IGNORECASE)
    text = _re.sub(r"</think(?:ing)?>", "", text, flags=_re.IGNORECASE).strip()
    if text.startswith("```"):
