...
def _active_cookbook_endpoint_ids() -> set[str]:
    ...
    for task in state.get("tasks") or []:
        ...
        if str(task.get("status") or "").lower() not in _COOKBOOK_ACTIVE_SERVE_STATUSES:
            continue
        ep_id = (
            task.get("_endpointId") or task.get("endpointId") or task.get("endpoint_id")
        )
        if ep_id:
            out.add(str(ep_id))
    return out
...
def _disable_stale_cookbook_local_endpoints(db) -> int:
    ...
    for ep in stale:
        ...
        logger.info(
            "Disabled stale Cookbook endpoint %s (%s @ %s)", ep.id, ep.name, ep.base_url
        )
    if touched_settings:
...
def _delete_orphaned_provider_auth(
    db, auth_id: str | None, exclude_ep_id: str | None = None
) -> bool:
...
def _normalize_endpoint_refresh_mode(
    value: Any, endpoint_kind: str = "auto", base_url: str = ""
) -> str:
...
def _model_display_name(model_id: str) -> str:
    if _is_mlx_deepseek_v4_shim_id(model_id):
        return (
            str(model_id or "").rstrip("/").split("/")[-1] or "DeepSeek-V4-Flash-4bit"
        )
...
def setup_model_routes(model_discovery):
                ...
                items.append(
                    {
                        ...
                        "models_extra_display": [
                            _model_display_name(mid) for mid in extra
                        ],
                        "endpoint_id": ep.id,
    ...
    def list_model_endpoints(request: Request) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for r in rows:
                ...
                if (
                    _picker_requires_pinning(base, kind)
                    and pinned
                    and not _has_explicit_pinned_models(r)
                ):
                ...
                status = (
                    "online"
                    if (all_models or visible or pinned)
                    else ("empty" if r.is_enabled else "offline")
                )
                results.append(
                    {
                        "id": r.id,
    ...
    def create_model_endpoint(
        ...
        refresh_mode = _normalize_endpoint_refresh_mode(
            model_refresh_mode, requested_kind, base_url
        )
        ...
        try:
            ...
            if existing:
                ...
                if (
                    incoming_model_type
                    and (getattr(existing, "model_type", None) or "llm")
                    != incoming_model_type
                ):
        ...
        try:
            ...
            if current_default_id:
                current_default_ep = (
                    db.query(ModelEndpoint)
                    .filter(ModelEndpoint.id == current_default_id)
                    .first()
                )
