...
def _sqlite_db_path(url) -> str | None:
...
class EmailAccount(TimestampMixin, Base):
    ...
    display_name = Column(
        String, nullable=True
    )  # "Hriday Ranka" — used in From: header
...
def _migrate_add_gallery_caption_column():
    ...
    try:
        ...
        columns = [
            row[1]
            for row in conn.execute("PRAGMA table_info(gallery_images)").fetchall()
        ]
        if columns and "caption" not in columns:
            conn.execute(
                "ALTER TABLE gallery_images ADD COLUMN caption TEXT DEFAULT ''"
            )
            conn.commit()
            logging.getLogger(__name__).info(
                "Migrated: added caption column to gallery_images"
            )
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"Migration gallery caption column failed: {e}"
        )
    finally:
        try:
            conn.close()
...
def _migrate_add_email_oauth_columns():
    ...
    try:
        with engine.connect() as conn:
            cols = [
                r[1] for r in conn.execute(text("PRAGMA table_info(email_accounts)"))
            ]
            for col, typedef in [
                ("oauth_provider", "TEXT"),
                ...
                if col not in cols:
                    conn.execute(
                        text(f"ALTER TABLE email_accounts ADD COLUMN {col} {typedef}")
                    )
            conn.commit()
    except Exception as e:
...
class CalendarEvent(TimestampMixin, Base):
    ...
    recurrence_exdates = Column(
        Text, default=""
    )  # JSON list of skipped occurrence starts
    ...
    caldav_sync_pending = Column(
        String, nullable=True
    )  # create | update | delete retry marker
...
def _scrub_legacy_chat_message_fts_media(conn) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).warning(
            f"chat_messages FTS media scrub skipped: {e}"
        )
        return
    ...
    try:
        ...
        for message_id, session_id, role, content in rows:
            conn.execute(
                "DELETE FROM chat_messages_fts WHERE message_id = ?", (message_id,)
            )
            conn.execute(
                """
                INSERT INTO chat_messages_fts(content, message_id, session_id, role)
        ...
        logging.getLogger(__name__).warning(
            f"chat_messages FTS media scrub failed: {e}"
        )
...
def _migrate_add_caldav_sync_columns():
    ...
    try:
        ...
        cal_columns = [
            row[1] for row in conn.execute("PRAGMA table_info(calendars)").fetchall()
        ]
        ...
        logging.getLogger(__name__).warning(
            f"CalDAV sync metadata migration failed: {e}"
        )
...
def _migrate_add_calendar_recurrence_exdates():
    ...
    try:
        ...
        if columns and "recurrence_exdates" not in columns:
            conn.execute(
                "ALTER TABLE calendar_events ADD COLUMN recurrence_exdates TEXT DEFAULT ''"
            )
        conn.commit()
    except Exception as e:
        logging.getLogger(__name__).warning(
            f"calendar_events recurrence_exdates migration failed: {e}"
        )
    finally:
        try:
            conn.close()
