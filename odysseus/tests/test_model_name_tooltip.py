...
def test_dropdown_item_has_title_tooltip():
    ...
    assert re.search(
        r"nameSpan\.title\s*=\s*m\.display", SRC
...
def test_header_indicator_has_title_tooltip():
    ...
    assert re.search(
        r"label\.title\s*=\s*modelId\b", body
...
def test_api_picker_dedupe_includes_endpoint_id():
    ...
    assert re.search(
        r"const seenKey = isApiEndpoint\s*\?", SRC
    ), "chat picker should dedupe API models by endpoint+model, not model id only"
    assert (
        "${item.endpoint_id || item.url || item.endpoint_name || 'api'}::${mid}" in SRC
    )
