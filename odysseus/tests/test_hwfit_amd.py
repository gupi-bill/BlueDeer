...
def _rocm_system(family="rdna", ram_gb=32.0, vram_gb=16.0):
    return {
        ...
        "backend": "rocm",
        "gpu_name": (
            "AMD Radeon RX 9060 XT" if family == "rdna" else "AMD Instinct MI300X"
        ),
        "gpu_vram_gb": vram_gb,
        "gpu_count": 1,
...
def test_only_gguf_models_recommended_on_consumer_rdna():
    ...
    assert (
        unservable == []
    ), f"{len(unservable)} non-GGUF models on RDNA, e.g. {unservable[:3]}"
...
def test_classify_amd_gfx_rdna_vs_cdna():
    ...
    for gfx, expected_family in cases.items():
        ...
        ), f"{gfx} -> {family}, expected {expected_family}"
...
def test_consumer_amd_cards_have_real_bandwidth():
    ...
    for name, expected_min in [
        ...
        assert (
            bw and bw >= expected_min
        ), f"{name}: {bw} GB/s (expected >= {expected_min})"
...
def test_no_vendor_specific_formats_on_consumer_rdna():
    ...
    assert any(
        bad.search(m["name"]) for m in get_models()
