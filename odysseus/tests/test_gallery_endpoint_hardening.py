...
SRC = (
    Path(__file__).resolve().parent.parent / "routes" / "gallery" / "gallery_routes.py"
)
...
def test_gallery_does_not_use_openai_substring_check():
    src = SRC.read_text()
    assert (
        '"api.openai.com" in base' not in src
...
def test_join_checked_accepts_known_paths():
    ...
    assert (
        j("http://localhost:7860/v1", "/images/img2img")
        == "http://localhost:7860/v1/images/img2img"
    )
    assert (
        j("http://localhost:7860", "/sdapi/v1/img2img")
        == "http://localhost:7860/sdapi/v1/img2img"
    )
    assert (
        j("https://api.openai.com/v1", "/images/edits")
...
def test_harmonize_uses_join_checked_for_target_construction():
    ...
    body = _function_source(src, "harmonize_image")
    assert (
        "_join_checked_gallery_endpoint" in body
...
def test_inpaint_endpoint_resolved_via_db_not_raw_input():
    ...
    assert (
        "_current_user_is_admin" not in body
    ...
    assert (
        'raise HTTPException(403, "Choose a registered image endpoint")' in body
...
def test_inpaint_outbound_base_not_from_request_body():
    ...
    body = _function_source(src, "inpaint_proxy")
    assert (
        "_join_checked_gallery_endpoint(requested_base," not in body
