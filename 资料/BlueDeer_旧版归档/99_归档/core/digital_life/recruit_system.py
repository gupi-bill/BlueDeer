...
# 冷却池参数
COOLDOWN_POOL_MAX_SIZE = 50
COOLDOWN_DEFAULT_DURATION = 300.0  # 5 分钟默认冷却
...
class RecruitSystem:
    ...
    def __init__(
        ...
        self._cd_pool: dict[str, float] = {}  # agent_id -> cooldown_until (monotonic)
    ...
    # ── 批量招募 ──────────────────────────────────────────────────
    def batch_recruit(self, agent_ids: list[str]) -> dict[str, str]:
        results = {}
        for aid in agent_ids:
            if aid in self._states:
                results[aid] = self.recruit_agent(aid)
            else:
    ...
    def recruit_agent(self, agent_id: str) -> str:
        ...
        if lf is None:
            return "failed"
        self.put_on_cooldown(agent_id)
        return "ok"

    # ── 冷却池管理 ──────────────────────────────────────────────
    @property
    def cooldown_pool(self) -> dict[str, float]:
        return dict(self._cd_pool)

    def put_on_cooldown(self, agent_id: str, duration: float | None = None) -> None:
        if len(self._cd_pool) >= COOLDOWN_POOL_MAX_SIZE:
            self._cd_pool.clear()
        dur = duration or COOLDOWN_DEFAULT_DURATION
        self._cd_pool[agent_id] = time.monotonic() + dur

    def _purge_cooldowns(self) -> None:
        now = time.monotonic()
        expired = [k for k, v in self._cd_pool.items() if v <= now]
        for k in expired:
            del self._cd_pool[k]
