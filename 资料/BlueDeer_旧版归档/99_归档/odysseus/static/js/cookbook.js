...
function _applyServerSelection(val) {
  ...
  // it silently reverted and 下载/scans hit the wrong server).
  _persistEnvState();
...
function _wireTabEvents(body) {
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      // 下载 to the wrong server. The dropdown the user sees is the truth.
    ...
    async function _loadLatest() {
      ...
      try {
        ...
        for (const m of models) {
          ...
          if (m.downloads) meta.push(`${m.downloads.toLocaleString()} 下载`);
  ...
  if (olToggle && olList) {
    ...
    async function _loadOllama(refresh = false) {
      ...
      try {
        ...
        if (!models.length) {
          olList.innerHTML = '<div class="hwfit-loading">无模型</div>';
          return;
        }
        let html = '';
        for (const m of models) {
        ...
        olList.innerHTML = '<div class="hwfit-loading">加载失败</div>';
      }
    }
...
export function _serverEntryHtml(s, i, defaultServer, forceRemote, isNew) {
  ...
  html += `<span style="width:100%;font-size:13px;font-weight:600;margin-bottom:3px;">Model Directory <span style="font-weight:400;opacity:0.5;font-size:11px;">— check the one 下载 should go to</span></span>`;
  for (let j = 0; j < modelDirs.length; j++) {
    ...
    const dlBtn = `<span class="cookbook-modeldir-dl${isTarget ? ' active' : ''}" title="${isTarget ? 'Downloads go here' : 'Send 下载 here'}" data-dl-dir="${esc(dirVal)}">${isTarget ? _MODELDIR_CHECK_ON : _MODELDIR_CHECK_OFF}</span>`;
...
function _renderRecipes() {
  ...
  html += '<button class="cookbook-tab" data-backend="Serve"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" stroke="none" style="vertical-align:-1px;margin-right:3px;"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>启动</button>';
  html += '<button class="cookbook-tab active" data-backend="Search"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="7 14 12 19 17 14"/><line x1="12" y1="19" x2="12" y2="5"/><line x1="5" y1="21" x2="19" y2="21"/></svg>下载</button>';
  html += '<button class="cookbook-tab" data-backend="Dependencies"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:3px;"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>依赖</button>';
  html += '<button class="cookbook-tab" data-backend="Settings"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" style="vertical-align:-1px;margin-right:3px;"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>设置</button>';
  html += '</div>';
  ...
  // silently sending 下载 to the wrong server. An empty selection means Local; the user
  ...
  html += `<input type="text" class="cookbook-dl-repo" id="cookbook-dl-repo" placeholder="org/模型名称, qwen2.5:14b, 或 HF URL" style="flex:1;min-width:0;" />`;
  html += `<button class="cookbook-btn cookbook-dl-btn" id="cookbook-dl-btn">下载</button>`;
  html += `</div>`;
  ...
  html += `<span style="pointer-events:none;flex:1;">适合你硬件的趋势模型</span>`;
  ...
  html += '<h2 style="margin:0;padding:0;line-height:1;">扫描 / 下载</h2>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc" style="margin-top:6px;">扫描你的硬件，找出可运行的模型。硬件信息已缓存；更换 GPU 后点击扫描按钮重新探测。</p>';
  ...
  html += '<input type="text" class="cookbook-field-input hwfit-search" id="hwfit-search" placeholder="搜索模型…" style="flex:1;" />';
  html += '</div>';
  ...
  html += '<option value="general" selected>标准</option>';
  html += '<option value="multimodal">视觉</option>';
  html += '<option value="image_gen">图像</option></select>';
  html += '<button type="button" class="cookbook-field-input hwfit-usecase-btn" data-hwfit-usecase-btn aria-haspopup="listbox" aria-expanded="false" title="模型类型">';
  ...
  html += '<option value="newest" selected>最新</option>';
  html += '<option value="fit">适配</option><option value="score">评分</option><option value="vram">显存</option>';
  html += '<option value="speed">Speed</option><option value="params">Params</option>';
  html += '<option value="context">Context</option></select>';
  html += '</div>';
  html += '<div class="hwfit-advanced-panel hidden" id="hwfit-advanced-panel" aria-label="扫描设置">';
  ...
  html += '<h2 style="margin:0;padding:0;line-height:1;">依赖</h2>';
  // Rebuild llama.cpp button moved into the llama_cpp dep row (see _depRow);
  // having it in the title polluted the section header.
  html += '<span style="font-size:10px;opacity:0.5;margin-left:auto;">服务器</span>';
