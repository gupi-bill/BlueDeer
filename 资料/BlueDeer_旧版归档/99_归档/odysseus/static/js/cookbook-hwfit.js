...
import {
  ...
  // (two _envState objects), which silently sent 下载 to the wrong server.
} from './cookbook.js';
...
export async function _hwfitFetch(fresh = false, opts = {}) {
  ...
  if (_cached) {
    ...
    if (canKeepPrevious) {
      ...
      loadingLbl.textContent = '正在加载模型列表…';
    ...
    if (!canKeepPrevious) {
      ...
      const loadingLbl = document.createElement('div');
      loadingLbl.textContent = fresh ? '正在扫描硬件…' : '正在加载模型…';
      loadingLbl.style.cssText = 'text-align:center;opacity:0.5;font-size:11px;';
      loadingDiv.appendChild(loadingLbl);
      setTimeout(() => {
        if (loadingLbl.isConnected) loadingLbl.textContent = fresh ? '正在扫描硬件…' : '正在加载模型列表…';
      }, 2000);
      list.innerHTML = '';
      list.appendChild(loadingDiv);
...
export function _hwfitRenderList(el, models) {
  ...
  if (!models.length) {
    ...
    let msg;
    if (hasFilters) msg = '无模型 match these filters — try clearing the search, use-case, quant, or engine.';
    else if (hasHw) msg = '无模型 fit — the hardware probe may have under-reported. Try Rescan.';
    else msg = '无模型 fit your hardware';
...
// clicking, which was sending 下载 to the wrong host.
...
export function _expandModelRow(row, modelData) {
  ...
  html += `<button class="cookbook-btn hwfit-dl-btn">下载</button>`;
...
export function _hwfitInit() {
  ...
  function _wireServerEntry(entry) {
    ...
    if (addDirBtn) addDirBtn.addEventListener('click', () => {
      ...
      tag.dataset.dir = dir;
      tag.innerHTML = `<span class="cookbook-modeldir-dl" title="Send 下载 here" data-dl-dir="${uiModule.esc(dir)}">${_MODELDIR_CHECK_OFF}</span> ${uiModule.esc(dir)} <span class="cookbook-modeldir-rm" title="Remove">\u2716</span>`;
      tag.querySelector('.cookbook-modeldir-rm').addEventListener('click', () => { tag.remove(); _syncServers(); });
  ...
  if (serverSelect && !serverSelect.dataset.bound) {
    ...
    serverSelect.addEventListener('change', () => {
      ...
      // it kept its old value and 下载 went to the wrong host even
