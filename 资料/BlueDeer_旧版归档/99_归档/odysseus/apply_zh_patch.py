...
def patch_cookbook():
    ...
    repls = [
        ...
        (
            "by pasting model link, or download directly in the Scan section below.",
            "粘贴模型链接下载，或直接在下方扫描区域下载。",
        ),
        (
            'placeholder="org/model-name, qwen2.5:14b, or HF URL"',
            'placeholder="org/模型名称, qwen2.5:14b, 或 HF URL"',
        ),
        (
            ">Trending models that fit your hardware</span>",
            ">适合你硬件的趋势模型</span>",
        ),
        # Scan / Download 区域
        (">Scan / Download<", ">扫描 / 下载<"),
        (
            ...
            "扫描你的硬件，找出可运行的模型。硬件信息已缓存；更换 GPU 后点击扫描按钮重新探测。",
        ),
        ...
        (
            'title="Refresh selected server hardware and cached models"',
            'title="刷新所选服务器硬件和缓存模型"',
        ),
        (
            'aria-label="Refresh selected server hardware and cached models"',
            'aria-label="刷新所选服务器硬件和缓存模型"',
        ),
        ...
        (
            ...
            '<span style="font-size:10px;opacity:0.5;margin-left:auto;">服务器</span>',
        ),
        # 状态/错误提示
