...
class DataConfig(BaseSettings):
    ...
    # Base directory
    base_dir: Path = Field(
        default=Path(get_app_root()), description="Base directory for the application"
    )

    # Data paths
    data_dir: Path = Field(
        default=Path(_DATA_DIR_CONST), description="Main data directory"
    )
    uploads_dir: Path = Field(
        default=Path(_DATA_DIR_CONST) / "uploads",
        description="Directory for uploaded files",
    )
    sessions_file: Path = Field(
        default=Path(_DATA_DIR_CONST) / "sessions.json",
        description="Sessions storage file",
    )
    memory_file: Path = Field(
        default=Path(_DATA_DIR_CONST) / "memory.json", description="Memory storage file"
    )
    personal_dir: Path = Field(
        default=Path(_DATA_DIR_CONST) / "personal_docs",
        description="Personal documents directory",
    )
    runbook_dir: Path = Field(
        default=Path(_DATA_DIR_CONST) / "personal_docs" / "runbook",
        description="Runbook directory",
    )
