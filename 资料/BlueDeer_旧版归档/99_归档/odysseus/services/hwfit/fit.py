...
_APPLE_VARIANT_KEYS_SORTED = sorted(
    APPLE_BANDWIDTH_BY_CORES.keys(), key=len, reverse=True
)
...
def analyze_model(
    ...
    budget = (
        unified_budget
        if unified_memory
        else (effective_vram if run_mode == "gpu" else available_ram)
    )
    ...
    if (
        fit_level == "good"
        and budget
        and required_gb <= budget * 0.50
        and run_mode != "cpu_offload"
    ):
        fit_level = "perfect"
        ...
        if term not in blob and (not norm or norm not in blob):
            if re.fullmatch(r"\d+(?:\.\d+)?b?", term):
                ...
                if (
                    wanted > 0
                    and actual > 0
                    and abs(actual - wanted) <= max(5.0, wanted * 0.08)
                ):
                    continue
            return False
    return True
...
def rank_models(
    ...
    for m in models:
        ...
        if (
            (apple_silicon or consumer_amd or is_windows)
            and not is_mlx
            and not (m.get("is_gguf") or m.get("gguf_sources"))
        ):
            continue
