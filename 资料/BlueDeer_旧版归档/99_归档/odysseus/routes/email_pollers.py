...
def _effective_settings_for_email_account(
    settings: dict, account_id: str | None
) -> dict:
...
def _ensure_away_reply_table():
    ...
    try:
        ...
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_email_away_msg ON email_away_replies(owner, account_id, message_id)"
        )
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_email_away_sender ON email_away_replies(owner, account_id, sender_addr, sent_at)"
        )
        conn.commit()
    finally:
        conn.close()
...
def _send_away_reply(
    ...
    if settings.get(
        "email_auto_reply_exclude_automated", True
    ) and _sender_is_automated(msg, sender_addr):
        return False, "automated sender"
    if _away_reply_already_sent(
        settings, account_owner, account_id, message_id, sender_addr
    ):
    ...
    subject_template = (
        settings.get("email_auto_reply_subject") or "(Away) {subject}"
    ).strip()
    if subject_template:
        original_subject = subject or ""
        reply_subject = (
            subject_template.replace("{subject}", original_subject).replace(
    ...
    outer["From"] = (
        email.utils.formataddr((display, from_addr)) if display else from_addr
    )
    outer["To"] = (
        email.utils.formataddr((sender_name, sender_addr))
        if sender_name
        else sender_addr
    )
    ...
    _record_away_reply(
        settings, account_owner, account_id, message_id, sender_addr, subject
    )
...
async def _auto_summarize_pass_single(
    ...
    auto_reply_draft = bool(
        auto_reply and settings.get("_email_auto_reply_draft_only", False)
    )
    auto_reply_away = bool(
        auto_reply and not auto_reply_draft and _away_reply_active(settings, account_id)
    )
    ...
    if (
        not auto_sum
        and not auto_reply_draft
        and not auto_reply_away
        and not auto_tag
        and not auto_spam
        and not auto_cal
    ):
    ...
    try:
        ...
        needs_llm = bool(
            auto_sum or auto_reply_draft or auto_tag or auto_spam or auto_cal
        )
        if needs_llm:
        ...
        for _entry in uid_list:
            ...
            try:
                ...
                need_away_reply = bool(
                    ...
                    and not _away_reply_already_sent(
                        settings, account_owner, account_id, message_id, _from_addr_only
                    )
                )
                    ...
                    try:
                        ...
                        if sent_away:
                            ...
                            _detail_lines.append(
                                f"away reply · {_folder}#{_uid_text} · {subject or '(no subject)'} — {away_detail}"
                            )
                        else:
                            _away_replies_skipped += 1
                            logger.info(
                                f"Away reply skipped for uid={uid}: {away_detail}"
                            )
                    except Exception as e:
                        _away_replies_failed += 1
                        _uid_text = uid.decode() if isinstance(uid, bytes) else str(uid)
                        _detail_lines.append(
                            f"away reply failed · {_folder}#{_uid_text} · {subject or '(no subject)'}"
                        )
        ...
        if auto_reply_away:
            parts.append(
                f"sent {_away_replies_sent} away repl"
                + ("y" if _away_replies_sent == 1 else "ies")
            )
    ...
    while True:
        try:
            ...
            await _asyncio.sleep(
                60 if settings.get("email_auto_reply", False) else 1800
            )
