...
def test_cerebras_not_self_hosted():
    ...
    llm_core = importlib.import_module("src.llm_core")
    assert (
        llm_core._is_self_hosted_openai_compatible("https://api.cerebras.ai/v1")
        is False
    )
...
def test_apply_local_cache_affinity_skips_cerebras():
    ...
    llm_core = importlib.import_module("src.llm_core")
    payload = {"messages": []}
    llm_core._apply_local_cache_affinity(
        payload, "https://api.cerebras.ai/v1", "test-session-123"
    )
