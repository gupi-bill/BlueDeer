...
class TestLmStudioSupportsVision:
    ...
    def _serve(self, monkeypatch, payload):
        monkeypatch.setattr(
            chat_helpers.httpx, "get", lambda url, timeout=None: _FakeResponse(payload)
        )

    def test_vision_true_from_capabilities(self, monkeypatch):
        self._serve(monkeypatch, self.PAYLOAD)
        assert (
            chat_helpers.lmstudio_supports_vision(
                self.URL, "qwen3.6-27b-custom-finetune"
            )
            is True
        )
...
class TestModelSupportsVision:
    ...
    def test_api_capability_overrides_name_heuristic(self, monkeypatch):
        ...
        monkeypatch.setattr(
            chat_helpers, "lmstudio_supports_vision", lambda url, m: True
        )
        assert (
            chat_helpers.model_supports_vision(
                "qwen3.6-27b-finetune", "http://localhost:1234/v1/chat/completions"
            )
            is True
        )
    ...
    def test_falls_back_to_name_when_endpoint_unknown(self, monkeypatch):
        ...
        monkeypatch.setattr(
            chat_helpers, "lmstudio_supports_vision", lambda url, m: None
        )
        assert (
            chat_helpers.model_supports_vision("qwen2-vl-7b", "http://host/v1") is True
        )
        assert (
            chat_helpers.model_supports_vision("plain-llm", "http://host/v1") is False
        )
