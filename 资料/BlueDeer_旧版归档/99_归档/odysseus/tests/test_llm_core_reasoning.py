...
def test_reasoning_field_emits_thinking_chunk(monkeypatch):
    ...
    assert any(
        (not d.get("thinking")) and d["delta"] == "Hello" for d in deltas
    ), deltas
...
def test_reasoning_content_field_still_supported(monkeypatch):
    ...
    assert any(
        d.get("thinking") and "older field" in d["delta"] for d in deltas
    ), deltas
    assert any(
        (not d.get("thinking")) and d["delta"] == "Answer" for d in deltas
    ), deltas
...
def test_thinking_field_emits_thinking_chunk(monkeypatch):
    ...
    assert any(
        d.get("thinking") and d["delta"] == "checking files" for d in deltas
    ), deltas
    assert any(
