...
def _default_state_params(primary: RGB) -> dict[RoleState, StateGlowParams]:
    ...
    return {
        RoleState.ONLINE: StateGlowParams(
            color=primary, layer=GlowLayer.MIDGROUND, brightness=1.0,
        ),
        RoleState.SLEEPING: StateGlowParams(
            color=primary.adjust_brightness(0.4),
            layer=GlowLayer.BACKGROUND, brightness=0.5,
        ),
        RoleState.ERROR: StateGlowParams(
            color=RGB(255, 60, 60),
            layer=GlowLayer.FOREGROUND, brightness=1.3, blink=True, pulse_frames=3,
        ),
        RoleState.ACHIEVEMENT: StateGlowParams(
            color=RGB(255, 200, 60),
            layer=GlowLayer.FOREGROUND, brightness=1.5, pulse_frames=4,
        ),
    }
...
_ROLE_PRESETS: dict[str, RoleGlowConfig] = {
    "squirrel": RoleGlowConfig(
        agent_id="squirrel", role="代码生成",
        primary_color=RGB(180, 100, 60), sprite="🐿",
        canvas_x=8, canvas_y=8,
    ),
    "fox": RoleGlowConfig(
        agent_id="fox", role="测试质量",
        primary_color=RGB(220, 120, 50), sprite="🦊",
        canvas_x=24, canvas_y=8,
    ),
    "hedgehog": RoleGlowConfig(
        agent_id="hedgehog", role="安全审计",
        primary_color=RGB(140, 100, 80), sprite="🦔",
        canvas_x=40, canvas_y=8,
    ),
    "beaver": RoleGlowConfig(
        agent_id="beaver", role="GitHub运维",
        primary_color=RGB(150, 110, 70), sprite="🦫",
        canvas_x=56, canvas_y=8,
    ),
    "owl": RoleGlowConfig(
        agent_id="owl", role="资料归档",
        primary_color=RGB(180, 160, 100), sprite="🦉",
        canvas_x=8, canvas_y=24,
    ),
    "soft_rabbit": RoleGlowConfig(
        agent_id="soft_rabbit", role="像素美术",
        primary_color=RGB(230, 180, 200), sprite="🐰",
        canvas_x=24, canvas_y=24,
    ),
    "fox_security": RoleGlowConfig(
        agent_id="fox_security", role="安全测试",
        primary_color=RGB(200, 80, 80), sprite="🦊",
        canvas_x=40, canvas_y=24,
    ),
    "fox_art": RoleGlowConfig(
        agent_id="fox_art", role="美术规范测试",
        primary_color=RGB(220, 140, 180), sprite="🦊",
        canvas_x=56, canvas_y=24,
    ),
    "hedgehog_static": RoleGlowConfig(
        agent_id="hedgehog_static", role="静态扫描",
        primary_color=RGB(160, 110, 90), sprite="🦔",
        canvas_x=8, canvas_y=40,
    ),
    "hedgehog_runtime": RoleGlowConfig(
        agent_id="hedgehog_runtime", role="运行时审计",
        primary_color=RGB(180, 130, 110), sprite="🦔",
        canvas_x=24, canvas_y=40,
    ),
    "hedgehog_keymgmt": RoleGlowConfig(
        agent_id="hedgehog_keymgmt", role="密钥管理",
        primary_color=RGB(120, 90, 140), sprite="🦔",
        canvas_x=40, canvas_y=40,
    ),
    "sparrow": RoleGlowConfig(
        agent_id="sparrow", role="状态播报",
        primary_color=RGB(120, 200, 240), sprite="🐦",
        canvas_x=56, canvas_y=40,
    ),
}
...
class RoleGlowRenderer:
    ...
    def render_role(
        ...
        if config is None:
            ...
            config = RoleGlowConfig(
                agent_id=agent_id, role="unknown",
                primary_color=RGB(150, 150, 150), sprite="?",
            )
        ...
        return self._renderer.render_glow(
            text, adjusted_color, params.layer, params.blink,
        )
    ...
    def render_role_with_label(
        ...
        config = _ROLE_PRESETS.get(agent_id) or RoleGlowConfig(
            agent_id=agent_id, role="unknown", sprite="?",
        )
    ...
    def render_pulse_frames(
        ...
        for i in range(frames_count):
            ...
            result.append(self._renderer.render_glow(
                config.sprite, adjusted, params.layer,
    ...
    def render_animation_sequence(
        self, agent_id: str, states: list[RoleState],
    ) -> list[str]:
        """渲染动画状态序列帧（多状态切换动画）。"""
        frames = []
        for state in states:
    ...
    def get_role_effect(self, agent_id: str, state: RoleState) -> dict[str, Any]:
        ...
        config = _ROLE_PRESETS.get(agent_id)
        if config is None:
            return {"color": RGB(150, 150, 150), "layer": GlowLayer.MIDGROUND,
                    "brightness": 1.0, "blink": False, "pulse_frames": 1}
        params = config.state_configs.get(state, config.state_configs[RoleState.ONLINE])
        return {
            "color": params.color, "layer": params.layer,
            "brightness": params.brightness, "blink": params.blink,
