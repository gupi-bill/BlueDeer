...
from modules.glow.alert_glow import AlertGlowLevel, AlertGlowRenderer
from modules.glow.role_glow import RoleGlowRenderer, RoleState
...
class DirtyTracker:
    ...
    def mark(self, x: int, y: int, w: int, h: int):
...
class TUIRenderer:
    ...
    def render_incremental(self, state: dict[str, Any],
                           changed_regions: list[tuple[int, int, int, int]] | None = None) -> str:
        ...
        for x, y, w, h in changed_regions:
            for row in range(y, min(y + h, self._h)):
                prev_line = self._prev_canvas._lines[row] if hasattr(self._prev_canvas, '_lines') and row < len(self._prev_canvas._lines) else ""
                cur_line = canvas._lines[row] if hasattr(canvas, '_lines') and row < len(canvas._lines) else ""
    ...
    def _compute_diff(self, prev: PixelCanvas, cur: PixelCanvas) -> list[tuple[int, str]]:
        changed = []
        for y in range(self._h):
            prev_line = prev._lines[y] if hasattr(prev, '_lines') and y < len(prev._lines) else ""
            cur_line = cur._lines[y] if hasattr(cur, '_lines') and y < len(cur._lines) else ""
            if prev_line != cur_line:
    ...
    def _draw_layout(self, canvas: PixelCanvas, state: dict[str, Any]) -> None:
        ...
        self._draw_task_board(
            canvas, mid_x + 1, body_y + 2, mid_w - 2, body_h - 3, state, global_frame,
        )

        canvas.draw_border(right_x, body_y, right_w, body_h, color=Color.SLATE)
        lb_idx = state.get("lb_index", DEFAULT_LB_INDEX)
        lb_name = LEADERBOARD_DIMENSIONS[lb_idx][1] if lb_idx < len(LEADERBOARD_DIMENSIONS) else "综合"
        canvas.draw_text(right_x + 1, body_y, f"\u256c排行榜[{lb_name}]", Color.AMBER)
        self._draw_leaderboard(
            canvas, right_x + 1, body_y + 2, right_w - 2, body_h - 3, state, lb_idx,
        )
    ...
    def _draw_title_bar(
        self, canvas: PixelCanvas, state: dict[str, Any], alert: bool,
    ) -> None:
        w = self._w
        ...
        canvas.draw_text_centered(1, f"\u2554\u2550\u2550 {title} \u2550\u2550\u2557", Color.GOLD)
        info = (
    ...
    def _draw_task_board(
        ...
        status_color = {
            "success": Color.SAGE, "failed": Color.SOFT_RED,
            "pending": Color.AMBER, "running": Color.SAGE,
        }
    ...
    def _draw_leaderboard(
        ...
        dim_key = LEADERBOARD_DIMENSIONS[lb_idx][0] if lb_idx < len(LEADERBOARD_DIMENSIONS) else "composite"
        ...
        dim_name = LEADERBOARD_DIMENSIONS[lb_idx][1] if lb_idx < len(LEADERBOARD_DIMENSIONS) else "综合"
        ...
        for i, entry in enumerate(sorted_board[:h - 1]):
            ...
            dim_val = sort_key(entry) if dim_key == "composite" else entry.get(dim_key, 0)
            if dim_key == "achievements":
                dim_val = len(entry.get("achievements", []))
            medal = ["\u2460", "\u2461", "\u2462", " ", " "][min(i, 4)]
            line = f"{medal}{aid:<7} {level:>3} {coins:>5} {dim_val:>6}"
            color = [Color.GOLD, Color.SILVER, Color.BRONZE, Color.SLATE, Color.SLATE][min(i, 4)]
    ...
    def _draw_extended_panels(
        ...
        panels = [
            ...
            ("\u256c\u5b89\u5168", state.get("security", {}), self._draw_security_panel),
    ...
    def _draw_dream_panel(
        self, canvas: PixelCanvas, x: int, y: int, w: int, h: int, data: dict[str, Any],
    ) -> None:
    ...
    def _draw_achievements(
        self, canvas: PixelCanvas, x: int, y: int, w: int, state: dict[str, Any],
    ) -> None:
        ...
        tier_color = {"bronze": Color.BRONZE, "silver": Color.SILVER, "gold": Color.GOLD}
        cur_x = x
    ...
    def _draw_decorations(
        self, canvas: PixelCanvas, w: int, h: int, global_frame: int,
    ) -> None:
        if w >= 20:
            canvas.draw_sprite(1, 1, TREE_SPRITE, {"#": TREE_COLOR, " ": None})
            canvas.draw_sprite(w - 6, 1, TREE_SPRITE, {"#": TREE_COLOR, " ": None})
        if w >= 40:
            canvas.draw_sprite(8, 0, LANTERN_SPRITE, {
                "\\": LANTERN_COLOR, "/": LANTERN_COLOR,
                "|": LANTERN_COLOR, "-": LANTERN_COLOR,
                "o": Color.ORANGE,
            })
            canvas.draw_sprite(w - 11, 0, LANTERN_SPRITE, {
                "\\": LANTERN_COLOR, "/": LANTERN_COLOR,
                "|": LANTERN_COLOR, "-": LANTERN_COLOR,
                "o": Color.ORANGE,
            })
