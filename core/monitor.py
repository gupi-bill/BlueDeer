...
class SystemMonitor:
    ...
    def evaluate_alerts(self, usage: dict[str, Any]) -> list[dict[str, Any]]:
        alerts: list[dict[str, Any]] = []
        ...
        if usage.get("memory_percent", 0) > 85:
            alerts.append({"level": "warning", "metric": "memory",
    ...
    async def _run_loop(self) -> None:
        while self._running:
            usage = self.resource_usage()
            alerts = self.evaluate_alerts(usage)
