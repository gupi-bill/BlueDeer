...
class Environment:
    ...
    def __init__(self) -> None:
        # Borg 核心：所有实例的 __dict__ 都指向同一个共享 dict
    ...
    def pop_dialogue_bubbles(self) -> list:
        ...
        Returns:
            气泡列表 [{id, speaker, text, target, expire_ts}]
        """
        now = time.time()
        with self._lock:
            ...
            taken = list(live)
            # 已取出 + 已过期的都从队列移除
            self.dialogue_bubbles = [b for b in self.dialogue_bubbles
                                     if b["expire_ts"] > now and b not in taken]
            return taken
    ...
    def tick_immersive_systems(self, dt: float = 1.0, router=None) -> None:
        ...
        - 情感氛围：每秒更新光环/粒子/区域累积
        - 记忆碎片：每秒清理过期 + 遗物飘移
        - 自发社交：每秒推进活跃对话气泡；扫描在系统内部按 600s 间隔自行节流

        Args:
            dt: 时间步长（秒）
            router: LLM router（自发社交生成对话用）
        """
        try:
            from core.digital_life.atmosphere_system import update_atmosphere
            update_atmosphere(self.population, dt)
        except Exception:
            pass
        try:
            from core.digital_life.memory_fragment import update_fragments
            update_fragments(dt)
        except Exception:
            pass
        try:
            from core.digital_life.spontaneous_social import scan_social, update_social
            update_social(dt)
            scan_social(self.population, env=self, router=router)
        except Exception:
            pass
        # commit 34：持久记忆 + 桌面宠物 + 生病急救
        try:
            from core.digital_life.persistent_memory import tick_persistent_memory
            tick_persistent_memory(dt)
        except Exception:
            pass
        try:
            from core.digital_life.desktop_pet import update_desktop_pet
            update_desktop_pet(self.population)
        except Exception:
            pass
        try:
            from core.digital_life.illness_system import update_illness
            weather = getattr(self, "weather", "sunny")
            season = getattr(self, "season", "spring")
            events = update_illness(self.population, dt, weather=weather, season=season)
            # 把疾病事件转为广播事件
            for ev in events:
                try:
        ...
        try:
            from core.digital_life.autobiographical_memory import tick_autobiography
            events = tick_autobiography(dt, population=self.population, router=router)
            for ev in events:
                try:
                    self.broadcast_event("autobio_event", ev)
                except Exception:
                    pass
        except Exception:
            pass
        try:
            from core.digital_life.diary_system import tick_diary
            events = tick_diary(dt, population=self.population, router=router)
            for ev in events:
                try:
                    self.broadcast_event("diary_event", ev)
                except Exception:
                    pass
        except Exception:
            pass
        try:
            from core.digital_life.work_artifacts import tick_artifacts
            events = tick_artifacts(dt, population=self.population, router=router)
            for ev in events:
                try:
                    self.broadcast_event("artifact_event", ev)
                except Exception:
                    pass
        except Exception:
            pass
