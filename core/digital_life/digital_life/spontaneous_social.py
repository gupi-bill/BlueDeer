...
class SpontaneousSocialSystem:
        ...
        for i, a in enumerate(candidates):
            ...
            for b in candidates[i + 1:]:
                ...
                if random.random() > prob:
                    continue
                # 触发对话
                self._start_dialogue(a, b, env, router)
    ...
    def _start_dialogue(self, a, b, env, router) -> None:
        """启动一次对话（异步生成对话内容）。"""
        # 异步生成对话内容，不阻塞主循环
        def _generate():
            ...
            with self._lock:
                self._active_dialogues.append(dialogue)
...
不要解释，直接输出对话。"""
    ...
    def _on_dialogue_finished(self, d: dict) -> None:
        """对话结束后的处理：归档 + 情感影响 + 碎片生成。"""
        d["end_ts"] = time.time()
        a = d["a"]
        b = d["b"]
        # 情感影响：双方 sadness 降低，joy 提升（被安慰/陪伴）
        try:
        ...
        try:
            ...
            pass
        # 生成摘要
        lines_text = " | ".join(f"{l['speaker']}：{l['text']}" for l in d["lines"])
        d["summary"] = lines_text[:200]
        # 归档（10 分钟后可查看）
        archive_entry = {
            "a": a._name_obj,
            "a_species": a.species,
