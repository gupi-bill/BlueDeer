...
class Deer(DigitalLifeForm):
    ...
    def _build_genome(self, override: dict | None) -> dict:
        ...
        2. 对数值字段做 ±5% 变异（保证个体差异）
        3. 从 sleep_config.json 注入作息字段（bedtime / wakeup_time / sleep_depth / wake_on_emergency）
        4. 合并 override（最高优先级）
        """
        # 1. 拷贝模板
        genome: dict = {}
        ...
        # 3. 注入作息配置（从 sleep_config.json 取 deer 默认）
        sleep_cfg = _load_sleep_config().get("deer", {})
        if sleep_cfg:
