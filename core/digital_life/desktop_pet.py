...
class DesktopPetState:
    ...
    def update_from_population(self, population: list) -> None:
        ...
        with self._lock:
            ...
            if should_switch:
                # 优先选有问题的（生病/能量低）
                candidates_sick = [lf for lf in population
                ...
                candidates_active = [lf for lf in population
                                     if getattr(lf, "_alive", True)
    ...
    def push_notification(self, title: str, body: str,
...
def push_desktop_notification(title: str, body: str,
                                priority: str = "normal") -> None:
...
def run_tray_server(console_url: str = "http://127.0.0.1:8080/",
    ...
    try:
        ...
        print("[tray] pystray 未安装，托盘功能不可用。安装：pip install pystray Pillow")
        return 1
    icon_img = _try_make_pixel_antler_icon()
    if icon_img is None:
        print("[tray] Pillow 未安装或图标生成失败，托盘功能不可用。")
        return 1
    ...
    icon = pystray.Icon("BlueDeer", icon_img,
                         "BlueDeer 森林公司 · 运行中", menu)
    try:
        icon.run()
        return 0
...
def start_tray_in_thread(console_url: str = "http://127.0.0.1:8080/",
