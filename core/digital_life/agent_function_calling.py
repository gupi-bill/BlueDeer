...
def cleanup_function_callers(max_age: float = 3600) -> int:
    """清理长时间未使用的 FunctionCaller 缓存。

    Args:
        max_age: 超过此秒数未使用的缓存被清理
    ...
    with _FUNCTION_CALLERS_LOCK:
        ...
        if len(_FUNCTION_CALLERS) > 50:
            ...
            for _ in range(excess):
                _FUNCTION_CALLERS.pop(next(iter(_FUNCTION_CALLERS)), None)
                removed += 1
    return removed
...
# ------------------------------------------------------------------
# commit 42：全局 function_registry / register_function / call_function
# ------------------------------------------------------------------
...
def call_function(name: str, *args, **kwargs) -> Any:
    entry = _FUNCTION_REGISTRY.get(name)
    if entry is None:
